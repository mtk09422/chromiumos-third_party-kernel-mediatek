...
{
	u32 rdata = 0;
	u32 addr = (u32)(rtc->addr_base + offset);
...
static void rtc_write(struct mt6397_rtc *rtc, u32 offset, u32 data)
{
	...
	addr = (u32)(rtc->addr_base + offset);

	if (offset < rtc->addr_range)
		regmap_write(rtc->regmap, addr, data);
}

static inline void rtc_busy_wait(struct mt6397_rtc *rtc)
{
...
static void rtc_set_lp_irq(struct mt6397_rtc *rtc)
{
	u16 irqen;

	irqen = rtc_read(rtc, RTC_IRQ_EN) | RTC_IRQ_EN_LP;
	rtc_write(rtc, RTC_IRQ_EN, irqen);
	rtc_write_trigger(rtc);
}

static void rtc_get_tick(struct mt6397_rtc *rtc, struct rtc_time *tm)
{
...
static void mtk_rtc_read_tick_time(struct mt6397_rtc *rtc, struct rtc_time *tm)
{
	rtc_get_tick(rtc, tm);
...
static void mtk_rtc_set_tick_time(struct mt6397_rtc *rtc, struct rtc_time *tm)
{
...
static void mtk_rtc_read_alarm_time(struct mt6397_rtc *rtc,
			struct rtc_time *tm, struct rtc_wkalrm *alm)
{
	u16 irqen, pdn2;
...
static void mtk_rtc_set_alarm_time(struct mt6397_rtc *rtc, struct rtc_time *tm)
{
	u16 irqen;
...
static irqreturn_t rtc_irq_handler_thread(int irq, void *data)
{
	...
	irqen = rtc_read(rtc, RTC_IRQ_EN) & ~RTC_IRQ_EN_AL;
	rtc_write(rtc, RTC_IRQ_EN, irqen);
	rtc_write_trigger(rtc);
...
static int mtk_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alm)
{
	...
	mutex_lock(&rtc->lock);
	mtk_rtc_read_alarm_time(rtc, tm, alm);
	mutex_unlock(&rtc->lock);
