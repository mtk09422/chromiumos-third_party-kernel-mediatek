...
#define PWRAP_GET_WACS0_REQ(x)      (((x)>>19) & 0x00000001)
...
static int pwrap_init_sidly(struct pmic_wrapper *wrp)
{
	...
	writel(WACS2, pwrap_base + PWRAP_HIPRIO_ARB_EN);
...
static int pwrap_init_cipher(struct pmic_wrapper *wrp)
{
	...
	writel(1, pwrap_base + PWRAP_CIPHER_EN);
...
static int pwrap_enable_wacs(struct pmic_wrapper *wrp)
{
	...
	writel(0x3f, pwrap_base + PWRAP_HIPRIO_ARB_EN);
...
static int pwrap_init(struct pmic_wrapper *wrp)
{
	...
	/* Enable DCM */
	writel(3, pwrap_base + PWRAP_DCM_EN);
	writel(0, pwrap_base + PWRAP_DCM_DBC_PRD);
...
static irqreturn_t pwrap_interrupt(int irqno, void *dev_id)
{
	...
	writel(WACS2, pwrap_base + PWRAP_HARB_HPRIO);
...
static int pwrap_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_threaded_irq(dev, irq,
			...
			IRQF_TRIGGER_HIGH, "mt8173-pwrap", wrp);
	if (ret) {
