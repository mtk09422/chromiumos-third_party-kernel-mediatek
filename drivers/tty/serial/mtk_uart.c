...
/* #define MSG_ERR(a...)   pr_err(a) */
/* #define DBG_TAG      "mtk_uart" */
...
static ssize_t mtk_uart_attr_show(struct kobject *kobj, struct attribute *attr,
			   char *buffer);
static ssize_t mtk_uart_attr_store(struct kobject *kobj, struct attribute *attr,
			    const char *buffer, size_t size);
static ssize_t mtk_uart_debug_show(struct kobject *kobj, char *page);
static ssize_t mtk_uart_debug_store(struct kobject *kobj, const char *page,
			     size_t size);
static ssize_t mtk_uart_sysrq_show(struct kobject *kobj, char *page);
static ssize_t mtk_uart_sysrq_store(struct kobject *kobj, const char *page,
			     size_t size);
static ssize_t mtk_uart_vffsz_show(struct kobject *kobj, char *page);
static ssize_t mtk_uart_vffsz_store(struct kobject *kobj, const char *page,
			     size_t size);
static ssize_t mtk_uart_conse_show(struct kobject *kobj, char *page);
static ssize_t mtk_uart_conse_store(struct kobject *kobj, const char *page,
			     size_t size);
static ssize_t mtk_uart_vff_en_show(struct kobject *kobj, char *page);
static ssize_t mtk_uart_vff_en_store(struct kobject *kobj, const char *page,
			      size_t size);
static ssize_t mtk_uart_lsr_status_show(struct kobject *kobj, char *page);
static ssize_t mtk_uart_lsr_status_store(struct kobject *kobj, const char *page,
				  size_t size);
/*---------------------------------------------------------------------------*/
static const struct sysfs_ops mtk_uart_sysfs_ops = {
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_attr_show(struct kobject *kobj, struct attribute *attr,
			   char *buffer)
{
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_debug_store(struct kobject *kobj, const char *buffer,
			     size_t size)
{
	int a, b, c, d;
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_sysrq_store(struct kobject *kobj, const char *buffer,
			     size_t size)
{
	struct mtuart_sysobj *obj =
	    container_of(kobj, struct mtuart_sysobj, kobj);
...
ssize_t mtk_uart_vffsz_show(struct kobject *kobj, char *buffer)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(obj->vffLen); idx++)
		len +=
		    scnprintf(buffer + len, PAGE_SIZE - len, "[%02d] %4d\n",
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_vffsz_store(struct kobject *kobj, const char *buffer,
			     size_t size)
{
#if defined(ENABLE_VFIFO)
	struct mtuart_sysobj *obj =
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_conse_store(struct kobject *kobj, const char *buffer,
			     size_t size)
{
	struct mtuart_sysobj *obj =
	    container_of(kobj, struct mtuart_sysobj, kobj);
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_vff_en_store(struct kobject *kobj, const char *buffer,
			      size_t size)
{
	int u1_tx, u1_rx, u2_tx, u2_rx, u3_tx, u3_rx, u4_tx, u4_rx;
	struct mtk_uart_setting *uart_setting;
	int res = sscanf(buffer, "%x %x %x %x %x %x %x %x",
			 &u1_tx, &u1_rx, &u2_tx, &u2_rx, &u3_tx, &u3_rx, &u4_tx,
			 &u4_rx);
...
/*---------------------------------------------------------------------------*/
ssize_t mtk_uart_lsr_status_store(struct kobject *kobj, const char *buffer,
				  size_t size)
{
...
static int __init mtk_uart_console_setup(struct console *co, char *options)
{
	...
	printk(KERN_ALERT DBG_TAG
	       "mtk console setup : co->index %d options:%s\n", co->index,
	       options);
	...
	printk(KERN_ALERT DBG_TAG
	       "mtk console setup : uart_set_option port(%d) "
...
{				/*NOTE: please save the phyiscal address in vff->dmahd */
	...
	for (idx = uart->nport * 2; idx < uart->nport * 2 + 2; idx++) {
		...
		if (vfifo->size)
			vfifo->addr =
			    dma_alloc_coherent(NULL, vfifo->size, &vfifo->dmahd,
					       GFP_DMA);
		else
			vfifo->addr = NULL;
...
static int mtk_uart_vfifo_delete(struct mtk_uart *uart)
{
	...
	for (idx = uart->nport * 2; idx < uart->nport * 2 + 2; idx++) {
		vfifo = &mtk_uart_vfifo_port[idx];
		if (vfifo->addr)
			dma_free_coherent(NULL, vfifo->size, vfifo->addr,
					  vfifo->dmahd);
		mtk_uart_vfifo_del_dbgbuf(vfifo);
		MSG_RAW("[%2d] %p (%04d) ;", idx, vfifo->addr, vfifo->size);
		vfifo->addr = NULL;
			      ...
			      struct mtk_uart_vfifo *vfifo)
{
	...
	case UART_TX_VFIFO_DMA:
		...
		if (!atomic_read(&vfifo->reg_cb)) {
			...
			ret =
			    request_irq(vfifo->irq_id,
					(irq_handler_t) mtk_vfifo_irq_handler,
					IRQF_LEVEL_TRIGGER_POLARITY, DRV_NAME,
					vfifo);
			if (ret)
				return ret;
...
static unsigned int mtk_uart_tx_empty(struct uart_port *port)
{
	...
	if (uart->tx_mode == UART_TX_VFIFO_DMA)
		return mtk_uart_vfifo_is_empty(uart->dma_tx.
					       vfifo) ? TIOCSER_TEMT : 0;
	else
#endif
...
static int mtk_uart_startup(struct uart_port *port)
{
	...
	if (uart->rx_mode == UART_RX_VFIFO_DMA) {
		...
		ret =
		    mtk_uart_dma_alloc(uart, &uart->dma_rx, uart->rx_mode,
				       uart->rx_vfifo);
	...
	if (uart->tx_mode == UART_TX_VFIFO_DMA) {
		...
		ret =
		    mtk_uart_dma_alloc(uart, &uart->dma_tx, uart->tx_mode,
				       uart->tx_vfifo);
	...
	if (uart->tx_mode == UART_TX_VFIFO_DMA) {
		...
		hrtimer_init(&uart->tx_vfifo->flush, CLOCK_MONOTONIC,
			     HRTIMER_MODE_ABS);
...
#endif
	int ret = 0;
	MSG(FUC, "%s: %8x, %d, %d\n", __func__, ser->flags, ser->custom_divisor,
	    uart->custom_baud);
...
#define GIC_DIST_PENDING_SET 0x200
	if (uart->tx_vfifo && uart->tx_mode == UART_TX_VFIFO_DMA) {
		irq_set_irq_type(uart->tx_vfifo->irq_id,
				 IRQF_LEVEL_TRIGGER_POLARITY);
		gic_pending =
		    readl(GIC_DIST_BASE + GIC_DIST_PENDING_SET +
...
#endif				/*CONFIG_PM */
/*---------------------------------------------------------------------------*/
static const struct dev_pm_ops mtk_uart_pm_ops = {
...
static int mtk_uart_init_ports(void)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		uart->port.mapbase = base;	/* for ioremap */
		...
		uart->base = (unsigned long)ioremap(base, 0x1000);
