...
#if defined __QNXNTO__
	/* 
	   Currently for an EVENTOBJECTWAIT bridge call, the Neutrino driver delegates the responsibility
	   of replying to the client, to the context which is initiated after a wakeup/resume call. This is
	   because the server considers continuing and waiting to listen to other clients' request instead
	   of blocking for the wait timeout/event to finish. This behavior may change once the server is
	   re-written to be able to accept multiple client requests.
	*/
	if (!IsMessageReplyDeferred())
	{
#endif
	/* 
