...
static PVRSRV_ERROR  TLTestCMD_DumpState (IMG_VOID)
{
	...
	{
		count++;
		PVR_LOG(("----- TL_SNODE[%d]: %p - psNext(%p) hDataEventObj(%p) psStream(%p) uiWRefCount(%d) psRDesc(%p)",
				count, psn, psn->psNext, psn->hDataEventObj, psn->psStream, psn->uiWRefCount, psn->psRDesc));
		...
		if (psn->psRDesc)
		{
			PVR_LOG(("------ TL_STREAM_DESC[%d]: %p - psNode(%p) ui32Flags(%x) hDataEvent(%p)",
				count, psn->psRDesc, psn->psRDesc->psNode, psn->psRDesc->ui32Flags, psn->psRDesc->hDataEvent));
		}
...
static IMG_VOID CleanupThread(IMG_PVOID pvData)
{
	...
	while (psThreadData->eOperation != TLT_OP_END)
	{
		...
		case TLT_OP_END: /* Used on Windows */
	 ...
	 * OP_END (Windows) or a kill signal (Linux).
