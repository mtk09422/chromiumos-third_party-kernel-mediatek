...
/* With only one instance of the timer correlation data the FW might read it while the Host is updating it.
 * We thus use an array of timer correlations (size 2 is enough to avoid race conditions) and in the worst
 * case the FW will read old data (which is still quite ok if the Host is updating it at that time) */
#define RGXFWIF_TIME_CORR_ARRAY_SIZE	2
...
#define RGXFWIF_INICFG_ALL					(0x000FFFDFU)
