...
#if 0
static const struct vm_operations_struct mediatek_drm_vm_ops = {
	.fault = mtk_gem_fault,
				...
				unsigned int flags,	unsigned long size)
{
	...
	if (flags == 0) {
		...
		if (dma_map_sg(dev->dev, sgt->sgl, sgt->nents,
				DMA_BIDIRECTIONAL) == 0) {
			pr_info("DBG_YT dma_map_sg failed\n");
			goto err_NEW;
		}
...
int mtk_drm_ioctl_gem_create(struct drm_device *dev, void *data,
				struct drm_file *file_priv)
{
	...
	dev_dbg(dev->dev, "DBG_YT mtk_drm_ioctl_gem_create\n");
	mtk_gem = mtk_drm_gem_create(dev, args->flags, args->size);
	if (IS_ERR(mtk_gem))
		return PTR_ERR(mtk_gem);
...
int mtk_drm_ioctl_gem_mmap(struct drm_device *dev, void *data,
				struct drm_file *file_priv)
{
	struct drm_gem_object *gem;
	struct drm_mtk_gem_mmap *args = data;
	unsigned int addr;


	gem = drm_gem_object_lookup(dev, file_priv, args->handle);
	if (!gem) {
		DRM_ERROR("failed to lookup gem object.\n");
		return -EINVAL;
	}

	/* gem->filp->f_op = &mtk_drm_gem_fops;
	gem->filp->private_data = obj; */
	...
	addr = vm_mmap(gem->filp, 0, args->size, PROT_READ | PROT_WRITE,
					MAP_SHARED, 0);
