...
/*	struct mtk_output *output = encoder_to_output(&encoder);
	...
	pr_info("DBG_YT mtk_drm_encoder_dpms mode = %d\n", mode);
...
int mtk_output_init(struct drm_device *dev)
{
	...
	pr_info("DBG_YT mtk_output_init1\n");
	/* only for bringup */
	...
	pr_info("DBG_YT mtk_output_init2\n");
	...
	pr_info("DBG_YT mtk_output_init3\n");
#else
...
int mtk_output_probe(struct mtk_output *output)
{
	...
	pr_info("DBG_jitao mtk_output_probe\n");
