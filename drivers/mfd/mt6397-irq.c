...
static const u16 mt6397_ack_reg[] = {
...
static void mt6397_irq_ack(struct irq_data *data)
{
	...
	regmap_write(mt6397->regmap,
			mt6397_ack_reg[irq_data->group], irq_data->mask);
}
...
static irqreturn_t mt6397_irq_thread(int irq, void *data)
{
	...
	if (regmap_read(mt6397->regmap,
		MT6397_INT_STATUS0, &irq_reg[GRP_INT_CON0]) < 0) {
		dev_err(mt6397->dev, "Failed to read INT_STATUS0\n");
		return IRQ_NONE;
	}

	if (regmap_read(mt6397->regmap,
		MT6397_INT_STATUS1, &irq_reg[GRP_INT_CON1]) < 0) {
	...
	for (i = 0; i < MT6397_IRQ_NR; i++) {
		if (irq_reg[mt6397_irqs[i].group] & mt6397_irqs[i].mask) {
			...
			regmap_write(mt6397->regmap,
				mt6397_ack_reg[mt6397_irqs[i].group],
				mt6397_irqs[i].mask);
		}
	}
	return IRQ_HANDLED;
