...
static int cros_ec_probe_all(struct cros_ec_device *ec_dev)
{
	...
	if (ret == 0) {
		...
		ec_dev->din_size = ec_dev->max_response +
		    sizeof(struct ec_host_response) + EC_MAX_RESPONSE_OVERHEAD;
		ec_dev->dout_size = ec_dev->max_request +
		    sizeof(struct ec_host_request) + EC_MAX_REQUEST_OVERHEAD;
...
int cros_ec_cmd_xfer(struct cros_ec_device *ec_dev, struct cros_ec_command *msg)
{
	...
	if (msg->insize > ec_dev->max_response) {
		dev_info(ec_dev->dev, "clamping message receive buffer\n");
	...
	if (msg->command < EC_CMD_PASSTHRU_OFFSET(1)) {
		if (msg->outsize > ec_dev->max_request) {
			dev_err(ec_dev->dev,
				...
				msg->outsize, ec_dev->max_request);
			mutex_unlock(&ec_dev->lock);
			return -EMSGSIZE;
		}
	} else {
		if (msg->outsize > ec_dev->max_passthru) {
			dev_err(ec_dev->dev,
				...
				msg->outsize, ec_dev->max_passthru);
			mutex_unlock(&ec_dev->lock);
			return -EMSGSIZE;
		}
	}
...
int cros_ec_register(struct cros_ec_device *ec_dev)
{
	...
	for_each_child_of_node(dev->of_node, node) {
		...
		dev_err(dev, "adding MFD sub-device %s\n", node->name);
