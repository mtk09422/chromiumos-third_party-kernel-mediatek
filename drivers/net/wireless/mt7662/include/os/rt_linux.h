...
/* George: special case for iNIC? */
#define AP_PROFILE_PATH			"/etc/Wireless/iNIC/iNIC_ap.dat"
#define AP_RTMP_FIRMWARE_FILE_NAME "/etc_ro/Wireless/iNIC/RT2860AP.bin"
#ifdef SINGLE_SKU_V2
/* George: not considering PCIe STA case */
#ifdef CONFIG_STA_SUPPORT
#error "no SINGLE_SKU_TABLE_FILE_NAME definition for PCIe STA+AP (RT2860STA)"
#else /* George: AP-only case */
#define SINGLE_SKU_TABLE_FILE_NAME	"/etc_ro/Wireless/iNIC/SingleSKU.dat"
#endif
#endif /* SINGLE_SKU_V2 */

#else /* CONFIG_RT_SECOND_IF_RF_OFFSET != 0x48000 */
/* George: general AP case? */
...
#endif /* CONFIG_RT_SECOND_IF_RF_OFFSET == 0x48000 */
...
#if 0 /* George: a neater form w/o considering in-house striptool */
#ifdef CONFIG_MULTI_CHANNEL
    #define MAX_PACKETS_IN_QUEUE				(2048)
#elif defined (DOT11_VHT_AC)
    #define MAX_PACKETS_IN_QUEUE				1024 /*(512)*/
#else
    #define MAX_PACKETS_IN_QUEUE				(512)
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,5,0)
#if defined(KMALLOC_MEM_SUPPORT) || defined(CONFIG_PRE_ALLOC)
