...
{
	DBGPRINT(RT_DEBUG_TRACE, (" @@@@ write_tmac_info_Data: wcid=%d , phy_mode=%d, pTransmit->field.BW=%u\n",pTxBlk->Wcid,phy_mode,pTransmit->field.BW));
}
#endif /* CONFIG_MULTI_CHANNEL */
...
#ifdef CONFIG_MULTI_CHANNEL
if (pAd->Mlme.bpkt_dbg)
	DBGPRINT(RT_DEBUG_TRACE, ("!! @@@@ write_tmac_info_Data : wcid %d mcs   %d bw  %d  basize %d \n",pTxBlk->Wcid,mcs,bw,basize));
#endif /* CONFIG_MULTI_CHANNEL */
		txwi_n->MCS = mcs;
		txwi_n->PHYMODE = phy_mode;
		txwi_n->BW = bw;
...
#ifdef CONFIG_MULTI_CHANNEL
	if (pAd->Mlme.bpkt_dbg)
		DBGPRINT(RT_DEBUG_TRACE, (" $$$$ write_tmac_info_Cache: wcid=%d , phy_mode=%d, pTransmit->field.BW=%u\n",pTxBlk->Wcid,phy_mode,pTransmit->field.BW));
#endif /* CONFIG_MULTI_CHANNEL */
...
#ifdef CONFIG_MULTI_CHANNEL
	if (pAd->Mlme.bpkt_dbg)
		DBGPRINT(RT_DEBUG_TRACE, ("$$$$ write_tmac_info_Cache: wcid %d mcs   %d bw  %d  basize %d \n",pTxBlk->Wcid,mcs,bw,basize));
#endif /* CONFIG_MULTI_CHANNEL */
...
INT get_pkt_rssi_by_rxwi(RTMP_ADAPTER *pAd, RXWI_STRUC *rxwi, INT size, CHAR *rssi)
{
	...
	if ( IS_MT7601(pAd))
	{
		rssi[0] = rxwi->RXWI_X.SNR2;
		rssi[1] = rxwi->RXWI_X.SNR1;
		rssi[2] = rxwi->RXWI_X.SNR0;
		return 0;
	}
	else
#endif /* MT7601 */
