	...
	IN MLME_QUEUE_ELEM *Elem)
{
						...
						ie_list, &LenVIE, pVIE))
	{
		...
		if (RTMPEqualMemory(ie_list->Ssid, "DIRECT-", 7))
			DBGPRINT(RT_DEBUG_TRACE, ("%s P2P_SCANNING: %s [%lu]\n", __FUNCTION__, ie_list->Ssid, Idx));
				...
				(WMODE_CAP_N(pAd->CommonCfg.PhyMode) && bAllowNrate))
			{
				...
				if (ie_list->AddHtInfoLen > 0)
				{
					...
					CentralChannel = get_cent_ch_by_htinfo(pAd,
		 			...
		 			DBGPRINT(RT_DEBUG_OFF, ("%s():AP HT-CtrlChannel=%d, CentralChannel=>%d\n",
					...
					(ie_list->vht_cap_len) && (ie_list->vht_op_len))
				{
					...
					if (vht_op->ch_width == 0) {
						...
						if (RTMP_GetCustomizedChannelBw(pAd, pAd->MlmeAux.Channel) != BW_80)
						{
							pAd->StaActive.SupportedPhyInfo.vht_bw = VHT_BW_2040;
						}
						else
								...
								pVIE))
	{
        ...
        */
		...
		if(MESH_ON(pAd) && (MeshIdLen != 0))
		{
			...
			InsertNeighborMP(pAd, RealRssi, bcn_ie_list->Bssid, bcn_ie_list->Channel, (UINT8)bcn_ie_list->AddHtInfo.AddHtInfo.RecomWidth,
				(UINT8)bcn_ie_list->AddHtInfo.AddHtInfo.ExtChanOffset, HostName, MeshId, MeshIdLen, bcn_ie_list->CapabilityInfo, (PUCHAR)pVIE, LenVIE, &MeshConfig);
...
#ifdef RT_CFG80211_SUPPORT
//                if (RTMPEqualMemory(ie_list->Ssid, "DIRECT-", 7))
                        DBGPRINT(RT_DEBUG_INFO, ("%s PASSIVE SCANNING: %s [%lu]\n", __FUNCTION__, bcn_ie_list->Ssid, Bssidx));
...
#ifdef MT7601
				if (IS_MT7601(pAd)){
					RxWI.RXWI_X.SNR2 = Elem->Rssi0;
					RxWI.RXWI_X.SNR1 = Elem->Rssi1;
					RxWI.RXWI_X.SNR0 = Elem->Rssi2;
					RxWI.RXWI_X.phy_mode = 0;
				}
				else
#endif /* MT7601 */
				{
...
#ifdef DOT11_VHT_AC
{
			...
			if (pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconCount < 50)
				pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconCount++;
			if(INFRA_ON(pAd) && (pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconSynced == FALSE) && pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconCount > 30)
			{
				RTMPSetTimer(&pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconSyncTimer, pAd->CommonCfg.BeaconPeriod - 15);
				pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconSynced = TRUE;
			}
#endif /* CERTIFICATION_SIGMA_SUPPORT */
			if (INFRA_ON(pAd) && (bcn_ie_list->DtimCount == 0) &&
				pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.bDoingPeriodChannelSwitch &&
				pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.bChannelSwitchInitiator)
			{
				BOOLEAN TimerCancelled;
				DBGPRINT(RT_DEBUG_WARN, ("101. DTIM %ld !!!\n", (jiffies * 1000) / OS_HZ));
				RTMPCancelTimer(&pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelSwitchTimer, &TimerCancelled);
				RTMPSetTimer(&pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelSwitchTimer, pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelStayTime);
				if(bcn_ie_list->MessageToMe)
					DBGPRINT(RT_DEBUG_ERROR, ("301. %ld DTIM MessageToMe !!!\n", (jiffies * 1000) / OS_HZ));
				if(pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.IamInOffChannel == TRUE)
				{
					DBGPRINT(RT_DEBUG_OFF, ("%ld Recieve Orig AP Beacon but FW didn't inform Back to Base, force back to base !!!\n", (jiffies * 1000) / OS_HZ));
