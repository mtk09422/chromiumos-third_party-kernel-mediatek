...
#endif /* ED_MONITOR */
#ifdef DYNAMIC_VGA_SUPPORT
INT Set_DynamicVGA_Enable(
        IN PRTMP_ADAPTER        pAd,
        IN PSTRING              arg);
#endif /* DYNAMIC_VGA_SUPPORT */
#ifdef CONFIG_MULTI_CHANNEL
INT Set_CH1StayTime_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_CH2StayTime_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_SwitchIdleTime_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_NULL_Frame_Count_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);
...
INT	Set_CH1_Channel_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT	Set_CH2_Channel_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_MCC_Start_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Set_Pkt_Dbg_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);
...
INT Show_CH1StayTime_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Show_CH2StayTime_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Show_SwitchIdleTime_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);

INT Show_NULL_Frame_Count_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg);
...
INT set_monitor_channel(PRTMP_ADAPTER ad, PSTRING arg)
{
	...
	INT set_add_acl(RTMP_ADAPTER *pAd, PSTRING arg)
	{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("%s:: Reach ACL MAC Addr Limit %d\n",	__FUNCTION__, ACL_LIST_LIMIT));
			return TRUE;
		}
		...
		DBGPRINT(RT_DEBUG_ERROR, ("%s:: ACL MAC Addr = %02x:%02x:%02x:%02x:%02x:%02x\n",	__FUNCTION__, PRINT_MAC(AclAddr)));
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("IF(ra%d) Set_TdlsChannelSwitchBWProc::(TdlsChannelSwitchBW=%d)\n",
		pObj->ioctl_if, pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TargetOffChannelBW));
	...
	IN	PSTRING			arg)
{
	...
	StayTime = simple_strtol(arg, 0, 10);

	pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelStayTime = StayTime;

	DBGPRINT(RT_DEBUG_ERROR, ("IF(ra%d) Set_CfgTdlsChannelSwitchBaseStayTimeProc::(BaseChannelStayTime=%d)\n",
		pObj->ioctl_if, pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelStayTime));
	...
	IN	PSTRING			arg)
{
	...
	if(pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.bChannelSwitchInitiator && pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.IamInOffChannel)
	{
		cfg_tdls_chsw_req(pAd,macAddr,pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannel,pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelBW);
	}
...
*/
	...
	if ((token != NULL) && (strlen(token)>1))
	{
		...
		*token = '\0';
		...
		//pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.bDoingPeriodChannelSwitch = TRUE;  //Wait util rcv CHSW resp to set this true
		pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannel = pAd->CommonCfg.Channel;
		pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelBW = pAd->CommonCfg.BBPCurrentBW;
        ...
        if(pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TargetOffChannelBW != 0)
    	{
			...
			pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TargetOffChannelExt = target_ext_ch_offset;

	        if(target_ext_ch_offset == EXTCHA_BELOW)
	           pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TargetOffChannel = TargetChannel - 2;
	        else
	           pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.TargetOffChannel = TargetChannel + 2;
    	}
		//cfg_tdls_prepare_null_frame(pAd,0,0,peermacAddr);
		cfg_tdls_chsw_req(pAd,peermacAddr,pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.OrigTargetOffChannel
		...
		//cfg_tdls_chsw_resp(pAd,peermacAddr,11000,19000,0);
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_CH1StayTime_Proc :: (channel_1st_staytime=%u) +  switch_idle_time=%u\n",
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_CH2StayTime_Proc::(channel_2nd_staytime=%u) +  switch_idle_time=%u \n",
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_SwitchIdleTime_Proc::(switch_idle_time=%u)\n", pAd->Mlme.switch_idle_time));
	return TRUE;
}
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_NULL_Frame_Count_Proc::(null_frame_count=%u)\n", pAd->Mlme.null_frame_count));
	return TRUE;
}
	...
	IN	PSTRING			arg)
{
	...
	if (start > 0)
	{
		...
		if (pAd->Mlme.channel_1st_channel <= 14) {
			...
			tssi_slope_offset_1st &= ~TSSI_PARAM2_SLOPE0_MASK;
			tssi_slope_offset_1st |= TSSI_PARAM2_SLOPE0(cap->tssi_0_slope_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_1st_channel)]);
			tssi_slope_offset_1st &= ~TSSI_PARAM2_SLOPE1_MASK;
			tssi_slope_offset_1st |= TSSI_PARAM2_SLOPE1(cap->tssi_1_slope_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_1st_channel)]);
			tssi_slope_offset_1st &= ~TSSI_PARAM2_OFFSET0_MASK;
			tssi_slope_offset_1st |= TSSI_PARAM2_OFFSET0(cap->tssi_0_offset_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_1st_channel)]);
			tssi_slope_offset_1st &= ~TSSI_PARAM2_OFFSET1_MASK;
			tssi_slope_offset_1st |= TSSI_PARAM2_OFFSET1(cap->tssi_1_offset_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_1st_channel)]);
		}
		...
		if (pAd->Mlme.channel_2nd_channel <= 14) {
			...
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_SLOPE0_MASK;
			tssi_slope_offset_2nd |= TSSI_PARAM2_SLOPE0(cap->tssi_0_slope_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_2nd_channel)]);
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_SLOPE1_MASK;
			tssi_slope_offset_2nd |= TSSI_PARAM2_SLOPE1(cap->tssi_1_slope_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_2nd_channel)]);
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_OFFSET0_MASK;
			tssi_slope_offset_2nd |= TSSI_PARAM2_OFFSET0(cap->tssi_0_offset_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_2nd_channel)]);
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_OFFSET1_MASK;
			tssi_slope_offset_2nd |= TSSI_PARAM2_OFFSET1(cap->tssi_1_offset_a_band[(INT32)get_chl_grp(pAd->Mlme.channel_2nd_channel)]);
		}
#ifdef RT_CFG80211_SUPPORT
	...
	else // GC dont' care NOA start time
	{
		...
		andes_mcc_start(pAd, pAd->Mlme.channel_1st_channel, pAd->Mlme.channel_2nd_channel, pAd->Mlme.channel_1st_bw,pAd->Mlme.channel_2nd_bw,
			...
			20,1,tssi_slope_offset_1st, tssi_slope_offset_2nd,0);
	...
	IN	PSTRING			arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("IF Set_Pkt_Dbg_Proc::(pAd->Mlme.bpkt_dbg=%d)\n", pAd->Mlme.bpkt_dbg));
	return TRUE;
}
	...
	IN	PSTRING			arg)
{
	...
	Channel = (UINT32)simple_strtol(arg, 0, 10);
...
#if 1 // michael set this central channel
	pAd->ApCfg.ApCliTab[ifIndex].CentralChannel = pAd->CommonCfg.CentralChannel;
#endif
	...
	DBGPRINT(RT_DEBUG_TRACE,("===> IF(apcli%d) Set_ApCli_Channel_Proc:: Channel=%d\n", ifIndex, Channel));
		...
		IN PSTRING				arg)
{
	...
	int i=0;
	ULONG port[64];
	NdisZeroMemory(pAd->WOW_Cfg.TcpPortV4, 64);
	while ((this_char = strsep((char **)&arg, ",")) != NULL && i<10) // TCP port max count is limited to 10
	{
		if (*this_char == '-')
		{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[%s] TCP port v4 [%d]:%d\n",__FUNCTION__, i, pAd->WOW_Cfg.TcpPortV4[i]));
		i++;
	}
		...
		IN PSTRING				arg)
{
	...
	int i=0;
	ULONG port[64];
	NdisZeroMemory(pAd->WOW_Cfg.TcpPortV6, 64);
	while ((this_char = strsep((char **)&arg, ",")) != NULL && i<10) // TCP port max count is limited to 10
	{
		if (*this_char == '-')
		{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[%s] TCP port v6[%d]:%d\n",__FUNCTION__, i, pAd->WOW_Cfg.TcpPortV6[i]));
		i++;
	}
		...
		IN PSTRING				arg)
{
	...
	int i=0;
	ULONG port[64];
	NdisZeroMemory(pAd->WOW_Cfg.UdpPortV4, 64);
	while ((this_char = strsep((char **)&arg, ",")) != NULL && i<10) // UDP port max count is limited to 10
	{
		if (*this_char == '-')
		{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[%s] UDP port v4[%d]:%d\n",__FUNCTION__, i, pAd->WOW_Cfg.UdpPortV4[i]));
		i++;
	}
		...
		IN PSTRING				arg)
{
	...
	int i=0;
	ULONG port[64];
	NdisZeroMemory(pAd->WOW_Cfg.UdpPortV6, 64);
	while ((this_char = strsep((char **)&arg, ",")) != NULL && i<10) // UDP port max count is limited to 10
	{
		if (*this_char == '-')
		{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[%s] UDP port v6[%d]:%d\n",__FUNCTION__, i, pAd->WOW_Cfg.UdpPortV6[i]));
		i++;
	}
		...
		IN PSTRING				arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("[%s] IP:",__FUNCTION__));
...
/*            	UCHAR Bssid[MAC_ADDR_LEN] = {0}; */
	...
	case OID_CUSTOMIZED_FEATURE_ENABLE:
	{
		...
		{
			INT feature = 0;
			Status = copy_from_user(&feature, wrq->u.data.pointer, wrq->u.data.length);
			if (Status == 0)
			{
				Set_Cust_Feature_Proc(pAd, feature);
			}
			DBGPRINT(RT_DEBUG_OFF, ("OID_CUSTOMIZED_FEATURE_ENABLE: Enable feature %d\n", feature));
		}
	...
	case OID_BW_CAPABILITY:
	{
		...
		else
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("OID_BW_CAPABILITY::Set HtBw=%d\n", HtBw));
		}
		break;
	}
	...
	case OID_HE_SET_ACL_POLICY:
	{
		...
		{
			INT acl_policy = 0;
			Status = copy_from_user(&acl_policy, wrq->u.data.pointer, wrq->u.data.length);
			if (Status == 0)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_SET_ACL_POLICY: %d\n", acl_policy));
	...
	case OID_HE_ADD_ACL_ADDR:
	{
		...
		{
			UCHAR macAddr[18] = {'\0'};
			Status = copy_from_user(macAddr, wrq->u.data.pointer, wrq->u.data.length);
			if (Status == 0)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_ADD_ACL_ADDR %s\n", macAddr));
	...
	case OID_HE_DEL_ACL_ADDR:
	{
		...
		else
		{
			...
			if (Status == 0)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_DEL_ACL_ADDR %s\n", macAddr));
	...
	case OID_HE_SET_REPLAY_COUNTER:
	{
		...
		else
		{
			...
			if (Status == 0)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_HE_SET_REPLAY_COUNTER ok\n"));
			}
		}
		break;
	}
	...
	case OID_HE_DO_ACTIVE_SCAN:
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_HE_DO_ACTIVE_SCAN: %d\n",pAd->StaCfg.bOnlyScanActiveChannel));
		break;
	}
	case OID_HE_CANCEL_SCAN:
	{
		if(CFG80211DRV_OpsScanRunning(pAd))
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_HE_CANCEL_SCAN: %d\n",pAd->StaCfg.bCancelScan));
		}
		else
		{
			DBGPRINT(RT_DEBUG_TRACE, ("Not in scanning, do nothing for cancel_scan\nSet::OID_HE_CANCEL_SCAN: %d\n",pAd->StaCfg.bCancelScan));
		}
	   ...
	   case OID_SET_MCC:
        {
            ...
            {
                INT Mcc_Enable = 0;
                Status = copy_from_user(&Mcc_Enable, wrq->u.data.pointer, wrq->u.data.length);
                if (Status == 0)
                {
...
ACM_BANDWIDTH_INFO BwInfo, *pInfo;
	...
	case RT_OID_802_11_QUERY_TXBF_TABLE:
		...
		case RT_OID_802_11_QUERY_MAP_REAL_TX_RATE:
			...
			wrq->u.data.length = sizeof(RateValue);
			Status = copy_to_user(wrq->u.data.pointer, &RateValue, wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE, ("Query::RT_OID_802_11_QUERY_LAST_TX_RATE (=%d)\n", RateValue));
			break;
		...
		{
#ifdef HE_TV_SUPPORT
			Status = copy_to_user(wrq->u.data.pointer, pAd->chipCap.FWImageName + 16, 12);
#else
			DBGPRINT(RT_DEBUG_ERROR, ("Query::RT_OID_802_11_QUERY_FIRMWARE_VERSION (=%lx)\n", pAd->FirmwareVersion));
            wrq->u.data.length = sizeof(ULONG);
		...
		case OID_HE_GET_ACL:
		{
			...
			for (i=0; i<acl.Num; i++)
			{
				NdisMoveMemory(&AclList[(i*MAC_ADDR_LEN)+1], &acl.Entry[i].Addr, MAC_ADDR_LEN);
			}
		...
		case OID_HE_GET_STA_INFO:
		{
			...
			for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
			{
				...
				if ((IS_ENTRY_CLIENT(pEntry) || (IS_ENTRY_APCLI(pEntry))) && (pEntry->Sst == SST_ASSOC))
				{
					sprintf(staInfo, "802.11%s %shz", pEntry->HTPhyMode.field.MODE==MODE_VHT?"ac":(pEntry->HTPhyMode.field.MODE==MODE_CCK?"b":(pEntry->HTPhyMode.field.MODE==MODE_OFDM?"g":"n")), get_bw_str(pEntry->HTPhyMode.field.BW));
					Status = copy_to_user(wrq->u.data.pointer, staInfo, sizeof(staInfo));
					DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_HE_GET_STA_INFO %s\n", staInfo));
				}
			}
			break;
		}
		case OID_HE_GET_PRODUCT_INFO:
		{
			...
			RtmpOsGetUsbDevManufacturerName(((POS_COOKIE)pAd->OS_Cookie)->pUsb_Dev, manufacturerName, sizeof(manufacturerName));
			RtmpOsGetUsbDevProductName(((POS_COOKIE)pAd->OS_Cookie)->pUsb_Dev, productName, sizeof(productName));
			RtmpOsGetUsbDevSerialNumber(((POS_COOKIE)pAd->OS_Cookie)->pUsb_Dev, serialNumber, sizeof(serialNumber));
			sprintf(productInfo, "\tVenderID: %04x\n\tProductID: %4x\n\tManufacturer: %s\n\tProduct: %s\n\tSerialNumber: %s\n",
						RtmpOsGetUsbDevVendorID(((POS_COOKIE)pAd->OS_Cookie)->pUsb_Dev),
						RtmpOsGetUsbDevProductID(((POS_COOKIE)pAd->OS_Cookie)->pUsb_Dev),
						manufacturerName,
						productName,
			...
			Status = copy_to_user(wrq->u.data.pointer, &productInfo, sizeof(productInfo));
			DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_HE_GET_PRODUCT_INFO %s\n", productInfo));
		}
#if defined(NEW_WOW_SUPPORT) && defined(RT_CFG80211_SUPPORT)
		case OID_HE_GET_WOW_STAT:
		{
			wrq->u.data.length = sizeof(char);
			Status = copy_to_user(wrq->u.data.pointer, &pAd->WOW_Cfg.bEnable, wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_HE_GET_WOW_STAT: %x\n", pAd->WOW_Cfg.bEnable));
		}
		break;
#endif /* defined(NEW_WOW_SUPPORT) && defined(RT_CFG80211_SUPPORT) */
#endif /* HE_TV_SUPPORT */
        ...
        IN PSTRING              arg)
{
    ...
    {
        DBGPRINT(RT_DEBUG_ERROR, ("Set Dynamic VGA to [%s]\n", Disabled?"DISABLED" : "ENABLED"));
        pAd->CommonCfg.DynamicVGA_Disabled = Disabled;
    }
    else
        DBGPRINT(RT_DEBUG_ERROR, ("Dynamic VGA already at [%s] state\n", Disabled?"DISABLED" : "ENABLED"));
