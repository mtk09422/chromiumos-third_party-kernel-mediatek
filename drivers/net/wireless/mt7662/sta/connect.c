		...
		*/
		...
		if ((pAd->MlmeAux.SsidBssTab.BssNr == 0) &&
...
#ifdef RT_CFG80211_SUPPORT
			{
				...
				if (!MAC_ADDR_EQUAL(pAd->cfg80211_ctrl.Cfg80211_ConnReqBssid, ZERO_MAC_ADDR))
				{
					if ((idx = BssTableSearchByBSSID(&pAd->MlmeAux.SsidBssTab, pAd->cfg80211_ctrl.Cfg80211_ConnReqBssid))
						 != (ULONG)BSS_NOT_FOUND)
					{
						DBGPRINT(RT_DEBUG_TRACE, ("%s: Found requested BSSID(%02x:%02x:%02x:%02x:%02x:%02x) @ BssIdx=%lu\n",
										__FUNCTION__, PRINT_MAC(pAd->cfg80211_ctrl.Cfg80211_ConnReqBssid), idx));
					}
					else
					{
						DBGPRINT(RT_DEBUG_TRACE, ("%s: Can not find requested BSSID(%02x:%02x:%02x:%02x:%02x:%02x) in table, "
										"use BssIdx=0\n",
										__FUNCTION__, PRINT_MAC(pAd->cfg80211_ctrl.Cfg80211_ConnReqBssid)));
						idx = 0;
					}
				}
		...
		else
		{
			...
			if ((pAd->StaCfg.wdev.AuthMode == Ndis802_11AuthModeAutoSwitch)
...
#ifdef RT_CFG80211_SUPPORT
				{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("%s: AUTH FAIL, DO NOT try next BSS\n", __FUNCTION__));
	...
	if (Elem->MsgType == MT2_ASSOC_CONF) {
		...
		if (Reason == MLME_SUCCESS) {
...
#ifdef RT_CFG80211_SUPPORT
			{
				BOOLEAN TimerCancelled;
				/* We should let wpa_supplicant do the roaming */
				DBGPRINT(RT_DEBUG_TRACE, ("%s: ASSOC FAIL, DO NOT try next BSS\n", __FUNCTION__));
				RTMPCancelTimer(&pAd->MlmeAux.BeaconTimer, &TimerCancelled);
				pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE;
				pAd->Mlme.SyncMachine.CurrState = SYNC_IDLE;
            RT_CFG80211_CONN_RESULT_INFORM(pAd, pAd->MlmeAux.Bssid, NULL, 0,
...
//		wdev->bw = pAd->CommonCfg.RegTransmitSetting.field.BW;
		...
		DBGPRINT(RT_DEBUG_TRACE, ("link uppAd->CommonCfg.RegTransmitSetting.field.BW %d pAd->CommonCfg.BBPCurrentBW %d \n",pAd->CommonCfg.RegTransmitSetting.field.BW,pAd->CommonCfg.BBPCurrentBW));

 //   if (pAd->CommonCfg.RegTransmitSetting.field.BW == BW_20) /* if our capability is only HT20 */
	...
	DBGPRINT(RT_DEBUG_TRACE, ("linkup bw %d  common %d central %d \n",wdev->bw,wdev->channel, wdev->CentralChannel ));
		...
		)
	{
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd))
		{
...

			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("#####pAd->Mlme.Now32 %lu pAd->StaCfg.LastBeaconRxTime %lu \n",pAd->Mlme.BeaconNow32,pAd->StaCfg.LastBeaconRxTime));
...
//	pAd->MlmeAux.InfraChannel = 0;
	...
	if (pAd->Mlme.bStartScc == TRUE)
	{
		...
		if(RTMP_CFG80211_VIF_P2P_GO_ON(pAd))
		{
			DBGPRINT(RT_DEBUG_TRACE, ("link down and switch to GO bw %d   pWdev->CentralChanne  %d \n",pWdev->bw,pWdev->CentralChannel));
