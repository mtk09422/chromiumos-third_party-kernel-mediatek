	...
	IN RXINFO_STRUC *pRxInfo)
{
	...
	if (eval_ant == select_ant)
	{
		if (pAd->CommonCfg.bTriggerTimer)
		{
			...
			/* calculate SNR */
			phymode = get_pkt_phymode_by_rxwi(pRxBlk->pRxWI);
		...
		STA_COLLECT_RX_ANTENNA_AVERAGE_RSSI(pAd, ConvertToRssi(pAd,
										(UCHAR)rxwi_x->SNR2, RSSI_0,
										rxwi_x->SNR1, rxwi_x->BW), 0);	/* Note: RSSI2 not used on RT73 */
...
//---Add by shiangf for debug
	...
	else if (pRxInfo->U2M)
	{
		...
		{
			pAd->StaCfg.BF_SNR[0] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR0;
			pAd->StaCfg.BF_SNR[1] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR1;
			pAd->StaCfg.BF_SNR[2] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR2;
		}
...
#ifdef DOT11_N_SUPPORT
static VOID STABuildCache802_11Header(
	IN RTMP_ADAPTER *pAd,
	IN TX_BLK *pTxBlk,
	IN UCHAR *pHeader)
{
...
static VOID STA_AMPDU_Frame_Tx(
	IN PRTMP_ADAPTER pAd,
	IN TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
...
static VOID STA_AMSDU_Frame_Tx(
	IN PRTMP_ADAPTER pAd,
	IN TX_BLK *pTxBlk)
{
	PUCHAR pHeaderBufPtr;
...
static VOID STA_ARalink_Frame_Tx(
	IN PRTMP_ADAPTER pAd,
	IN TX_BLK * pTxBlk)
{
	PUCHAR pHeaderBufPtr;
...
static VOID STA_Fragment_Frame_Tx(
	IN RTMP_ADAPTER *pAd,
	IN TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
