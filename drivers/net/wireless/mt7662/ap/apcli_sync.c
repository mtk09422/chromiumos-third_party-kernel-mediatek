	...
	IN MLME_QUEUE_ELEM *Elem) 
{
								...
								pVIE))
	{
		...
		*/
        ...
        RT_CFG80211_SCANNING_INFORM(pAd, Bssidx, Elem->Channel /*ie_list->Channel*/, (UCHAR *)Elem->Msg, Elem->MsgLen, RealRssi);
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE */
		...
		if (matchFlag)
		{
			...
			*/
			...
			DBGPRINT(RT_DEBUG_TRACE, ("=====================> %d, %d, %d, %d\n", pApCliEntry->MlmeAux.BeaconPeriod, ie_list->HtCapabilityLen, pApCliEntry->wdev.DesiredHtPhyInfo.bHtEnable,
				...
				!(pAd->CommonCfg.HT_DisallowTKIP && IS_INVALID_HT_SECURITY(wdev->WepStatus)))
			{
				...
				if (ie_list->AddHtInfoLen > 0)
				{
					...
					CentralChannel = get_cent_ch_by_htinfo(pAd, &ie_list->AddHtInfo,
		 			...
		 			DBGPRINT(RT_DEBUG_TRACE, ("APCLI PeerBeaconAtJoinAction HT===>CentralCh = %d, ControlCh = %d\n",
									CentralChannel, ie_list->AddHtInfo.ControlChan));
				}
	...
	IN USHORT ifIndex)
{
	...
	else
	{
		...
		else
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("[apcli_sync] force out a ProbeRequest by broadcast but not unicast...\n"));			
			ApCliMgtMacHeaderInit(pAd, &Hdr80211, SUBTYPE_PROBE_REQ, 0,
				BROADCAST_ADDR, BROADCAST_ADDR, ifIndex);
			}
			else	
