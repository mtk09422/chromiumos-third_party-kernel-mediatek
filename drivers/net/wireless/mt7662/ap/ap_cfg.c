...
#if 0
			sprintf(msg+strlen(msg)," MaxCap:%-10s", get_phymode_str(pEntry->MaxHTPhyMode.field.MODE));
				...
				else
				{
							...
							(pApCliEntry->wdev.AuthMode >= Ndis802_11AuthModeWPA))
					{
						...
						DBGPRINT(RT_DEBUG_TRACE, ("Set ApCli::OID_802_11_ADD_WEP (id=0x%x, Len=%d-byte), %s\n",
									...
									pMacEntry->PortSecured == WPA_802_1X_PORT_SECURED ? "Port Secured":"Port NOT Secured"));
				}
			}
    	...
    	case RT_SET_APD_PID:
			{
    			...
    			else
    			{
					...
					DBGPRINT(RT_DEBUG_TRACE, ("RT_SET_APD_PID::(ApdPid=%lu(%p))\n", apd_pid, pObj->apd_pid));
				}
    		}
			break;
	...
	IN INT cmd)
{
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		os_alloc_mem(pAd, (UCHAR **)&pBuf, BssBufSize);
			...
			*/
					if ((pAd->ApCfg.ApCliTab[ifIndex].wdev.AuthMode >= Ndis802_11AuthModeWPA) && (pMacEntry->PortSecured != WPA_802_1X_PORT_SECURED))
					{
		...
		case OID_802_11_CURRENTCHANNEL:
			...
			wrq->u.data.length = sizeof(UCHAR);
			DBGPRINT(RT_DEBUG_TRACE, ("sizeof UCHAR=%zd, channel=%d \n", sizeof(UCHAR), pAd->CommonCfg.Channel));
...
static VOID RTMPAPIoctlBBP32(RTMP_ADAPTER *pAd, RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	{
		DBGPRINT(RT_DEBUG_INFO, ("copy to user:msg[len=%zu]=%s\n", strlen(msg), msg));
	}
#endif /* RELEASE_EXCLUDE */
...
INT Set_ApCli_Ssid_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	if(strlen(arg) <= MAX_LEN_OF_SSID)
	{
			...
			apcli_entry->PSKLen > 0)
		{
			...
			{
				AtoH((PSTRING) PskKey, apcli_entry->PMK, 32);
			}
#else
