	...
	INOUT ADD_HT_INFO_IE *pAddHtInfo)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, (" @@@@ ApCliCheckHt :  peer bw=%u,  desired bw=%u, aux bw=%u \n", pAddHtInfo->AddHtInfo.RecomWidth, rt_ht_cap->ChannelWidth,aux_ht_cap->HtCapInfo.ChannelWidth));
...
#ifdef P2P_SUPPORT
static VOID ComposeP2PNullFrame(
	IN PRTMP_ADAPTER pAd,
	IN PAPCLI_STRUCT pApCliEntry)
{
   	struct wifi_dev *wdev;
		...
		COPY_MAC_ADDR(pApCliEntry->LastBssid, pAd->MlmeAux.Bssid);
			...
			*/
			...
			if (pAd->bApCliCertTest == TRUE)
			{
				...
				//RTMP_IO_WRITE32(pAd, RLT_PBF_MAX_PCNT, 0x1F3FBF9F);
			}
#endif /* APCLI_CERT_SUPPORT */
#endif
...
VOID ApCliIfMonitor(RTMP_ADAPTER *pAd)
{
	...
	for(index = 0; index < MAX_APCLI_NUM; index++)
	{
		...
		if (pApCliEntry->Valid == TRUE)
		{
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("ApCliIfMonitor: IF(apcli%d) - PortNotSecured after 30s.\n", index));
				bForceBrocken = TRUE;
 			}
...
#define APCLI_LOST_BEACON_TIME 4
			...
			if (RTMP_TIME_AFTER(pAd->Mlme.Now32 , (pApCliEntry->ApCliRcvBeaconTime + (APCLI_LOST_BEACON_TIME * OS_HZ))))
			{
	...
	IN UCHAR FromWhichBSSID)
{
	...
	*/
	...
	DBGPRINT(RT_DEBUG_INFO, ("IF-apcli%d : B/M-cast frame pRxWI->WirelessCliID=%d, keyidx=%d \n",
				pEntry->wdev_idx, pRxBlk->wcid, pRxBlk->key_idx));
#endif /* RELEASE_EXCLUDE */
