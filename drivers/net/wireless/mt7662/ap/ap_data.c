...
INT APSendPacket(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket)
{
	...
	if (/*(RTMP_CFG80211_VIF_P2P_CLI_ON(pAd) && INFRA_ON(pAd)) ||*/ pAd->Mlme.bStartMcc == TRUE)
	{
		map_to_edca2 =1;
		if (pAd->Mlme.bpkt_dbg)
		{
			DBGPRINT(RT_DEBUG_ERROR, ("@@@@ : %s map_to_edca2 , Wcid=%u, pMacEntry->apidx=%u\n", __FUNCTION__,wdev->if_dev, Wcid,pMacEntry->apidx));
		}
	}
...
#ifdef RELEASE_EXCLUDE
					DBGPRINT(RT_DEBUG_INFO, ("%s(%s):Drop M/B-cast packet before device secured!\n",
...
#ifdef RELEASE_EXCLUDE
					DBGPRINT(RT_DEBUG_INFO, ("%s(%s):Drop unicast packet before entry secured!\n",
								__FUNCTION__, RtmpOsGetNetDevName(wdev->if_dev)));
#endif /* RELEASE_EXCLUDE */
					RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
					return NDIS_STATUS_FAILURE;
...
#ifdef RELEASE_EXCLUDE
			if (IS_ENTRY_CLIENT(pMacEntry) && apidx < pAd->ApCfg.BssidNum) {
	...
	*/
	...
	{	/* The addr3 of WDS packet is Destination Mac address and Addr4 is the Source Mac address. */
		...
		if (RTMP_GET_MESH_SOURCE(pPacket) == MESH_PROXY)
			COPY_MAC_ADDR(pHeader80211->Octet, pAd->MeshTab.wdev.if_addr);
		else
...
//+++Add by shiang for debug
		...
		if (pEntry)
		{
			if ((pEntry->Sst == SST_ASSOC) && IS_ENTRY_CLIENT(pEntry))
			{
				...
				offset = MAC_WCID_BASE + (pEntry->wcid * HW_WCID_ENTRY_SIZE);
				DBGPRINT(RT_DEBUG_OFF, ("Invalid One: Check if ASIC WCID table(0x%x) has this entry (wcid=%d)!\n",
									offset, pEntry->wcid));
...
static BOOLEAN APCheckTkipMICValue(
	IN	PRTMP_ADAPTER	pAd,
...
static VOID APRxDataFrameAnnounce(
	IN RTMP_ADAPTER *pAd,
	IN MAC_TABLE_ENTRY *pEntry,
	IN RX_BLK *pRxBlk,
	IN UCHAR FromWhichBSSID)
...
if (0/*!(pRxInfo->Mcast || pRxInfo->Bcast)*/){
	...
	hex_dump("DataFramePayload", pRxBlk->pData , (pRxBlk->DataSize > 64 ? 64 :pRxBlk->DataSize));
}
...
//---Add by shiangf for debug
	...
	*/
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1))
	{
		do
		{
			...
			if (VALID_WCID(pRxBlk->wcid)
				&& IS_ENTRY_MESH(&pAd->MacTab.Content[pRxBlk->wcid]))
			{
				...
				FromWhichBSSID = pEntry->wdev_idx + MIN_NET_DEVICE_FOR_MESH;
				break;
			}
	...
	if (RTMP_SA_WORK_ON(pAd))
	{
		...
		if (pRxBlk->rssi[0])
		{
			pEntry->curRSSI[0] += (pRxBlk->rssi[0] & 0xff);
			pEntry->cntRSSI[0]++;
		}
		else
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Err!Receive A frame with RSSI0=%d!TotalZeroCnt=%d!\n",
						pRxBlk->rssi[0], pEntry->rssi_zero_cnt[0]));
		}
		if (pRxBlk->rssi[1])
		{
			pEntry->curRSSI[1] += (pRxBlk->rssi[1] & 0xff);
			pEntry->cntRSSI[1]++;
		}
		else
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Err!Receive A frame with RSSI1=%d!TotalZeroCnt=%d!\n",
						pRxBlk->rssi[1], pEntry->rssi_zero_cnt[1]));
		}

#ifdef DOT11N_SS3_SUPPORT
		if (pRxBlk->rssi[2])
		{
			pEntry->curRSSI[2] += (pRxBlk->rssi[2] & 0xff);
			pEntry->cntRSSI[2]++;
		}
		...
		if (pRxBlk->snr[0])
		{
			pEntry->sumSNR[0] += (pRxBlk->snr[0] & 0xff);
			pEntry->cntSNR[0]++;
		}
		if (pRxBlk->snr[1])
		{
			pEntry->sumSNR[1] += (pRxBlk->snr[1] & 0xff);
			pEntry->cntSNR[1]++;
		}
#ifdef DOT11N_SS3_SUPPORT
		if (pRxBlk->snr[2])
		{
			pEntry->sumSNR[2] += (pRxBlk->snr[2] & 0xff);
			pEntry->cntSNR[2]++;
		}
...
// TODO: shiang@PF#2, is this atheros protection still necessary here????
...
#endif /* MT7601 */
		RssiSample = ConvertToRssi(pAd, (UCHAR)pRxBlk->rssi[0], RSSI_0);
	...
	{
		pEntry->BF_SNR[0] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR0;
		pEntry->BF_SNR[1] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR1;
		pEntry->BF_SNR[2] = BF_SNR_OFFSET + pRxWI->RXWI_O.BF_SNR2;
	}
...
//---Add by shiangf for debug
	...
	*/
...
#if 0 // TODO: Is there any side-effect if we don't use  this code segment? 20120803
		if (pEntry && IS_ENTRY_APCLI(pEntry))
#else
		if (pEntry)
#endif
	...
	if (RTMP_SA_WORK_ON(pAd))
	{
		...
		{
			pEntry->curRSSI[0] += (pRxWI->RXWI_O.RSSI0 & 0xff);
			pEntry->cntRSSI[0]++;
		}
		else
		{
			pEntry->rssi_zero_cnt[0]++;
			printk("Err!Receive A frame with RSSI0=%d!TotalZeroCnt=%d!\n", pRxWI->RXWI_O.RSSI0, pEntry->rssi_zero_cnt[0]);
		}
		if (pRxWI->RXWI_O.RSSI1)
		{
			pEntry->curRSSI[1] += (pRxWI->RXWI_O.RSSI1 & 0xff);
			pEntry->cntRSSI[1]++;
		}
		else
		{
			pEntry->rssi_zero_cnt[1]++;
			printk("Err!Receive A frame with RSSI1=%d!TotalZeroCnt=%d!\n", pRxWI->RXWI_O.RSSI1, pEntry->rssi_zero_cnt[1]);
		}

#ifdef DOT11N_SS3_SUPPORT
		if (pRxWI->RXWI_O.RSSI2)
		{
			pEntry->curRSSI[2] += (pRxWI->RXWI_O.RSSI2 & 0xff);
			pEntry->cntRSSI[2]++;
		}
#endif /* DOT11N_SS3_SUPPORT */
		...
		{
			pEntry->sumSNR[0] += (pRxWI->RXWI_O.SNR0 & 0xff);
			pEntry->cntSNR[0]++;
		}
		if (pRxWI->RXWI_O.SNR1)
		{
			pEntry->sumSNR[1] += (pRxWI->RXWI_O.SNR1 & 0xff);
			pEntry->cntSNR[1]++;
		}
#ifdef DOT11N_SS3_SUPPORT
		if (pRxWI->RXWI_O.SNR2)
		{
			pEntry->sumSNR[2] += (pRxWI->RXWI_O.SNR2 & 0xff);
			pEntry->cntSNR[2]++;
		}
		...
		{
			pEntry->sumPreSNR[0] += (pRxWI->RXWI_O.BF_SNR0 & 0xff);
			pEntry->cntPreSNR[0]++;
		}
		if (pRxWI->RXWI_O.BF_SNR1)
		{
			pEntry->sumPreSNR[1] += (pRxWI->RXWI_O.BF_SNR1 & 0xff);
			pEntry->cntPreSNR[1]++;
		}
		if (pRxWI->RXWI_O.BF_SNR2)
		{
			pEntry->sumPreSNR[2] += (pRxWI->RXWI_O.BF_SNR2 & 0xff);
			pEntry->cntPreSNR[2]++;
		}
#endif /* DOT11N_SS3_SUPPORT */
		pEntry->saRxCnt++;
