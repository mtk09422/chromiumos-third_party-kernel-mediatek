...
========================================================================
*/
static VOID ATEDisableAsicProtect(
	IN		PRTMP_ADAPTER	pAd)
{
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_RT6352(pAd))
	{
		...
		/* it is done in RT6352_RTMPReadTxPwrPerRate */
...
INT Set_ATE_GET_EFUSE_MODE_Proc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PSTRING			arg)
{
	CHAR value;
...
static VOID ATEAsicTemperCompensation(
	IN	PRTMP_ADAPTER 		pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
			...
			if (pATEInfo->PeriodicRound % (ATE_TASK_EXEC_MULTIPLE * 4) == 0)
			{
				if ((pAd->ate.bAutoVcoCal == TRUE) && (pATEInfo->Mode == ATE_TXFRAME))
				{
...
#endif /* MT76x0 */
					{
						...
						if (IS_RT3593(pAd))
						{
							// TODO: recover it in the near future
