...
static INT mt76x2_ate_tx_pwr_handler(
	IN RTMP_ADAPTER *ad,
	IN char index)
{
	PATE_INFO pATEInfo = &(ad->ate);
...
static INT mt76x2_ate_tx_pwr_Evaluation(
	IN RTMP_ADAPTER *ad)
{
	PATE_INFO pATEInfo = &(ad->ate);
...
static VOID mt76x2_ate_rx_vga_init(
	IN PRTMP_ADAPTER		ad)
{
...
static VOID mt76x2_ate_set_tx_rx_path(
    IN PRTMP_ADAPTER ad)
{
	PATE_INFO   pATEInfo = &(ad->ate);
...
static INT	mt76x2_set_ate_tx_bw_proc(
	IN	PRTMP_ADAPTER	ad,
	IN	PSTRING			arg)
{
...
#if 0
static VOID mt76x2_adjust_tssi_offset(
...
static VOID mt76x2_ate_asic_adjust_tx_power(
	IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->chipCap.tssi_enable) &&
		...
		if (pAd->ate.Channel < 14) {
			...
			tssi_slope_offset &= ~TSSI_PARAM2_SLOPE0_MASK;
			tssi_slope_offset |= TSSI_PARAM2_SLOPE0(cap->tssi_0_slope_a_band[(INT32)get_chl_grp(pAd->ate.Channel )]);
			tssi_slope_offset &= ~TSSI_PARAM2_SLOPE1_MASK;
			tssi_slope_offset |= TSSI_PARAM2_SLOPE1(cap->tssi_1_slope_a_band[(INT32)get_chl_grp(pAd->ate.Channel )]);
			tssi_slope_offset &= ~TSSI_PARAM2_OFFSET0_MASK;
			tssi_slope_offset |= TSSI_PARAM2_OFFSET0(cap->tssi_0_offset_a_band[(INT32)get_chl_grp(pAd->ate.Channel )]);
			tssi_slope_offset &= ~TSSI_PARAM2_OFFSET1_MASK;
			tssi_slope_offset |= TSSI_PARAM2_OFFSET1(cap->tssi_1_offset_a_band[(INT32)get_chl_grp(pAd->ate.Channel )]);
...
static INT	mt76x2_set_ate_tx_freq_offset_proc(
	IN	PRTMP_ADAPTER	ad,
	IN	PSTRING			arg)
{
	u32 freq_offset = 0;
...
#else
static VOID mt76x2_ate_asic_calibration(
	IN PRTMP_ADAPTER pAd, UCHAR ate_mode)
{
