    ...
    IN UINT idx)
{
	...
	{
#if 0
    	RTMP_RA_GRP_TB *pCurTxRate, *pFurTxRate;
		UCHAR	        TxRateIdx;
		CHAR	        mcs[24];
		CHAR	        RssiOffset = 0;
		...
		DBGPRINT(0,("MCS Rate %d/%d with Low Pakcet \n", pEntry->CurrTxRateIndex, TxRateIdx));

    	pCurTxRate = PTX_RA_GRP_ENTRY(pTable, pEntry->CurrTxRateIndex);
        pFurTxRate = PTX_RA_GRP_ENTRY(pTable, TxRateIdx);

    	if(pCurTxRate->Mode < pFurTxRate->Mode)
    	{
            return;
        }
    	else if((pCurTxRate->Mode == pFurTxRate->Mode) && (pEntry->CurrTxRateIndex < TxRateIdx))
    	{
    	    return;
    	}
#endif
	...
	else
	{
		...
		if (pEntry->wcid >= 1 && pEntry->wcid <= 8)
		{
			...
			DBGPRINT(RT_DEBUG_INFO ,("%s():TxErrRatio(Wcid:%d, MCS:%d, Hw:0x%x-0x%x, Sw:0x%x-%x)\n",
					__FUNCTION__, pEntry->wcid, pEntry->HTPhyMode.field.MCS,
					(UINT32)HwTxCnt, (UINT32)HwErrRatio, (UINT32)TxTotalCnt, (UINT32)TxErrorRatio));
