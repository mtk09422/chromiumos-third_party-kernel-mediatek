	...
	if (RTMP_SA_WORK_ON(pAd))
	{
		...
		*/
        ...
        if (TxTotalCnt <= 15)
        {
			...
			if (TxRateIdx != pEntry->CurrTxRateIndex
#if 0 // it will make BF test item to be failed at BroadComm testbed and hope BF will always keep being applied even if packet number is lower
#ifdef TXBF_SUPPORT
...
#if 0 // BF will keep being applied even if MCS rate is changed
#ifdef TXBF_SUPPORT
			pEntry->phyETxBf = pEntry->phyITxBf = FALSE;
#endif /* TXBF_SUPPORT */
#endif
		...
		else
		{
			...
			if (pEntry->wcid >= 1 && pEntry->wcid <= 8)
			{
				...
				if (HwTxCnt)
					...
					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA, ("%s():TxErrRatio(wcid:%d, MCS:%d, Hw:%lu-%lu, Sw:%lu-%lu)\n", 
						__FUNCTION__, pEntry->wcid, pEntry->HTPhyMode.field.MCS, 
						HwTxCnt, HwErrRatio, TxTotalCnt, TxErrorRatio));
