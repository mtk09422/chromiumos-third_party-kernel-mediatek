...
UCHAR RTMP_GetCustomizedChannelBw(
	...
	IN UCHAR Channel)
{
	PCH_DESC pChDesc = (Channel <= CFG80211_NUM_OF_CHAN_2GHZ)?Country_Region_ChDesc_2GHZ[(pAd->CommonCfg.CountryRegion & 0x7f)].pChDesc:Country_Region_ChDesc_5GHZ[(pAd->CommonCfg.CountryRegionForABand & 0x7f)].pChDesc;
			
	while (pChDesc->FirstChannel)
	{
		if ((Channel >= pChDesc->FirstChannel) && (Channel <= (pChDesc->FirstChannel + ((pChDesc->NumOfCh - 1) * 4))))
		{
			DBGPRINT(RT_DEBUG_INFO, ("Customized %s for Reg[%d] CH[%d] is found\n", pChDesc->ChannelWidth==BW_80?"BW80":(pChDesc->ChannelWidth==BW_40?"BW40":"BW20"), pAd->CommonCfg.CountryRegion & 0x7f, Channel));
			return pChDesc->ChannelWidth;
		}	
		pChDesc++;
	}
	DBGPRINT(RT_DEBUG_ERROR, ("No matching bw result for Reg[%d] CH[%d]\n", pAd->CommonCfg.CountryRegion & 0x7f, Channel));
	return 0;
}
