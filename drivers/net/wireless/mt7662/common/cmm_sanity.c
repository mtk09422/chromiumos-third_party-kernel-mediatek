        ...
        switch(pEid->Eid)
	{
		...
		case IE_CHANNEL_SWITCH_ANNOUNCEMENT:
			if(pEid->Len == 3)
            {
                ...
                DBGPRINT(RT_DEBUG_TRACE, ("%s, CSA IE: Mode %d, New Chnl %d, Cnt %d\n", __FUNCTION__, 
                                          pAd->CommonCfg.ChannelSwitchMode, pAd->CommonCfg.NewChannel, 
                                          pAd->CommonCfg.ChannelSwitchCount));
#endif               
...
SanityCheck:
	...
	if ((Sanity != 0x7) || ( bWscCheck == FALSE))
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s Sanity Failed, Sanity=%02x, MsgChannel=%u, pAd->LatchRfRegs.Channel=%u\n", __FUNCTION__, Sanity, MsgChannel,pAd->LatchRfRegs.Channel));
#endif /* CONFIG_MULTI_CHANNEL */
		DBGPRINT(RT_DEBUG_LOUD, ("%s() - missing field, Sanity=0x%02x\n", __FUNCTION__, Sanity));
		return FALSE;
	}
		...
		switch(pEid->Eid)
		{
			...
			case IE_EXT_CHANNEL_SWITCH_ANNOUNCEMENT:
                ...
                if (pEid->Len == 4)
                {
                    ...
                    DBGPRINT(RT_DEBUG_TRACE, ("%s, ECSA IE: Mode %d, Op Class %d, New Chnl %d, Cnt %d\n", __FUNCTION__, 
                                               pAd->CommonCfg.ExtChannelSwitchMode, pAd->CommonCfg.ExtNewOperatingClass,
                                               pAd->CommonCfg.ExtNewChannel, pAd->CommonCfg.ExtChannelSwitchCount));
                }
				DBGPRINT(RT_DEBUG_ERROR, ("Trigger LinkDown due to receiving CSA (ChannelSwitchAnnouncement) from AP\n"));
				LinkDown(pAd, FALSE);
#endif  
	        ...
	        case IE_VENDOR_SPECIFIC:
					...
					)
                {
					...
					*/
					...
					if (pEntry == NULL)
						pEntry = MacTableInsertEntry(pAd, pAddr2, wdev, BSS0, OPMODE_STA, FALSE);
