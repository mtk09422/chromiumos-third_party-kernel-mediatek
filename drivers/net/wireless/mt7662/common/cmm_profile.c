...
/*
    ...
    ========================================================================
*/
static PSTRING RTMPFindSection(IN const PSTRING buffer)
{
    STRING temp_buf[32];
    PSTRING  ptr;
...
/*
    ...
    ========================================================================
*/
static INT RTMPGetKeyParameterWithOffset(
    IN  PSTRING   key,
    OUT PSTRING   dest,
...
static void parse_ack_policy(RTMP_ADAPTER *pAd, PSTRING tmp, const PSTRING src)
{
	PSTRING ptr;
	INT32 i;
...
static inline void RTMPSetSTACipherSuites(RTMP_ADAPTER *pAd, NDIS_802_11_ENCRYPTION_STATUS WepStatus)
{
...
NDIS_STATUS	RTMPSetSingleSKUParametersCustomer(
	IN RTMP_ADAPTER *pAd)
{
	PSTRING buffer;
	PSTRING		readline, token;
    ...
    if (RTMP_GetSingleSkuPathByCountryRegion(pAd, singlesku_path, sizeof(singlesku_path)) < 0)
    {
        ...
        strncpy(singlesku_path, SINGLE_SKU_TABLE_FILE_NAME, strlen(SINGLE_SKU_TABLE_FILE_NAME));
        DBGPRINT(RT_DEBUG_TRACE, ("%s: Could not find dynamic pwr table, use default one=[%s]\n", __FUNCTION__, singlesku_path));
    }
    else
        DBGPRINT(RT_DEBUG_TRACE, ("%s: Found dynamic pwr table=[%s]\n", __FUNCTION__, singlesku_path));
	...
	if (IS_FILE_OPEN_ERR(srcf))
	{
        ...
        if (strcmp(singlesku_path, SINGLE_SKU_TABLE_FILE_NAME) != 0)
        {
            ...
            DBGPRINT(RT_DEBUG_ERROR, ("Failed to open dynamic pwr table[%s], fallback to default one\n", singlesku_path));
            memset(singlesku_path, 0, sizeof(singlesku_path));
            strncpy(singlesku_path, SINGLE_SKU_TABLE_FILE_NAME, strlen(SINGLE_SKU_TABLE_FILE_NAME));
            srcf = RtmpOSFileOpen(singlesku_path, O_RDONLY, 0);
            if (!IS_FILE_OPEN_ERR(srcf))
			...
			if ( !strncmp(readline, "ch", 2) )
			{
				...
				else if (pwr->StartChannel <= 14)
				{
					for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT_LENGTH ; i++ )
					{
						...
						if ( token == NULL )
							break;
						pwr->PwrVHT40All[i] = simple_strtol(token, 0, 10) *2;
					}
				}
#endif /* DOT11_VHT_AC */
				/* VHT20 MCS 8 */
...
#if 0
	{
		...
		DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("VHT20_MCS8: "));
			for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT20_LENGTH ; i++ )
			{
				DBGPRINT(RT_DEBUG_TRACE,("%d ", ch->PwrVHT20[i]));
			}
			DBGPRINT(RT_DEBUG_TRACE,("\n"));

			DBGPRINT(RT_DEBUG_TRACE, ("VHT40_MCS8_9: "));
			for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT40_LENGTH ; i++ )
			{
				DBGPRINT(RT_DEBUG_TRACE,("%d ", ch->PwrVHT40[i]));
			}
			DBGPRINT(RT_DEBUG_TRACE,("\n"));
		}
	}
