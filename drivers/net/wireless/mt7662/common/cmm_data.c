...
static VOID dump_txblk(TX_BLK *pTxBlk)
{
	...
	{	int j;
		...
		if (pPacket)
		{
			...
			DBGPRINT(RT_DEBUG_TRACE,("\t\t[%d]:ptr=0x%x, Len=%d!\n", i, (UINT32)(ULONG)(GET_OS_PKT_DATAPTR(pPacket)), GET_OS_PKT_LEN(pPacket)));
...
/*
	...
	========================================================================
*/
static void AP_QueuePsActionPacket(
	...
	IN PNDIS_PACKET pPacket)
{
	...
	if (pAd->CommonCfg.MlmeTransmit.field.MODE == MODE_CCK && 
      NdisEqualMemory(pAd->cfg80211_ctrl.P2PCurrentAddress,pHeader_802_11->Addr2, MAC_ADDR_LEN)) 
	{
		if (pAd->CommonCfg.MlmeTransmit.field.MODE == MODE_CCK)
		{	
			pAd->CommonCfg.MlmeTransmit.field.MODE = MODE_OFDM;
	...
	else
	{
		...
		if (pTxBlk->TxFrameType == TX_LEGACY_FRAME)
		{
			...
			{	/* Specific packet, i.e., bDHCPFrame, bEAPOLFrame, bWAIFrame, need force low rate.*/
                ...
                if (RTMP_GET_PACKET_LOWRATE(pPacket) && pAd->ApCfg.ApCliTab[0].CtrlCurrState == APCLI_CTRL_CONNECTED)
                {
                    /* Follow Mgmt rate */
...
static BOOLEAN CanDoAggregateTransmit(
	IN RTMP_ADAPTER *pAd,
	...
	for (QueIdx=sQIdx; QueIdx >= eQIdx; QueIdx--)
	{
		...
		while (1)
		{
                ...
                pAd->StaCfg.bChannelSwitchCountingDown)
            {
                ...
                RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_SUCCESS);
                DEQUEUE_UNLOCK(&pAd->irq_lock, bIntContext, IrqFlags);
                DBGPRINT(RT_DEBUG_TRACE, ("Go into silent mode due to CSA IE!\n"));
                continue;
            }
#endif
			pTxBlk = &TxBlk;
...
#define ENTRY_RETRY_INTERVAL	(100 * OS_HZ / 1000)
...
#ifdef RELEASE_EXCLUDE
				DBGPRINT(RT_DEBUG_LOUD, ("TxRing(%d) full, SwQueue Len = %u, packet required frag =%d!\n",
								QueIdx, pAd->TxSwQueue[QueIdx].Number, RTMP_GET_PACKET_FRAGMENTS(pPacket)));
#ifdef RTMP_MAC_PCI
				DBGPRINT(RT_DEBUG_INFO, ("DeqPkt:NoEnoughFreeTxD[%d](TX_CIDX=%u,TxSwFreeIdx=%u)!\n",
			...
			if (Status != NDIS_STATUS_SUCCESS)
			{
				DBGPRINT(RT_DEBUG_TRACE,("%s(): return failed!!!\n", __FUNCTION__));
				break;
			}
#endif /* 0 */
		}
	...
	OUT UCHAR *pQueIdx)
{
	...
	*/
	...
	if (pMacEntry->bSamsungForcePriority)
		up = 7;
	...
	IN CHAR Index)
{
	...
	if (NState == NDIS_STATUS_SUCCESS)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("@@@ %s : pNullFr->Addr1 =%02x:%02x:%02x:%02x:%02x:%02x\n",__FUNCTION__ , PRINT_MAC(pNullFr->Addr1)));
		...
		if (IS_ENTRY_APCLI(pEntry))
		{
			...
			//yiwei
		DBGPRINT(RT_DEBUG_TRACE, ("@@@ %s : pNullFr->Addr2 =%02x:%02x:%02x:%02x:%02x:%02x\n",__FUNCTION__ , PRINT_MAC(pNullFr->Addr2)));

			#if 1 //yiwei MCC, use apcli for p2p0 cli
		...
		DBGPRINT(RT_DEBUG_TRACE, ("@@@ write to buffer pAd->NullBufOffset[%d]=0x%x\n",Index,pAd->NullBufOffset[Index]));
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		...
		if ((pAd->CommonCfg.bSWRxAntDiversity)
			...
			if ((pRxBlk->pRxInfo->U2M)
				...
				&& (MAC_ADDR_EQUAL(&pAd->CommonCfg.Bssid, &pHeader->Addr2))))
			{
				STA_COLLECT_RX_ANTENNA_AVERAGE_RSSI(pAd, ConvertToRssi(pAd, (UCHAR) pRxBlk->rssi[0], RSSI_0), 0);	/* Note: RSSI2 not used on RT73 */
...
//+++Add by shiang for debug
		...
		if (pHeader->FC.Type == FC_TYPE_DATA) {
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Dump the RxBlk info\n"));
