...
/*
	...
	========================================================================
*/
static NTSTATUS RTUSBFirmwareOpmode(RTMP_ADAPTER *pAd, UINT32 *pValue)
{
	NTSTATUS	Status;
	...
	IN	UINT32			TransferBufferLength)
{
	...
	if(in_interrupt())
	{
		...
		if(pAd->WOW_Cfg.bInSuspendMode & (WOW_IN_SUSPENDING | WOW_SUSPEND_COMPLETE))
		{
...
static NTSTATUS TdlsSendCHSWSetupHdlr(IN PRTMP_ADAPTER pAd, IN PCmdQElmt CMDQelmt)
{
	...
	else
	{
		...
		DBGPRINT(RT_DEBUG_ERROR,("START CH SW!! Initiator:(%d) Base:(%d) Target:(%d) BW:(%d,%d) EXT(%d,%d),SW Time:(%d) SW Timeout:(%d) OffChannelStayTime:(%d) start_time_tsf:(%ld)\n"
				,pTdlsCHSWParam->initiator,pTdlsCHSWParam->basech,pTdlsCHSWParam->offch,
			pTdlsCHSWParam->bw_base,
            pTdlsCHSWParam->bw_off,
            pTdlsCHSWParam->ext_base,
            pTdlsCHSWParam->ext_off,
