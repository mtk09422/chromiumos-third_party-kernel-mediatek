...
#ifdef MT7601
		if (IS_MT7601(pAd))
			beaconLen = pAd->BeaconTxWI.TXWI_X.MPDUtotalByteCnt;
		else
#endif /* MT7601 */
...
VOID AsicDelWcidTab(RTMP_ADAPTER *pAd, UCHAR wcid_idx) 
{
	...
	if (wcid_idx == WCID_ALL) {
		cnt_s = 2; //Fix station could not ping ap successfully when sta&p2p GO concurrent.
		//cnt_s = 0;
		cnt_e = (WCID_ALL - 1);
	} else {
#ifdef MCS_LUT_SUPPORT
	...
	IN USHORT num)
{
	...
	if (IS_MT76x2(pAd))
	{
		...
		{
			/* use default */
			RTMP_IO_WRITE32(pAd, AMPDU_MAX_LEN_20M1S_MCS0_7, 0xAAA99887);
			RTMP_IO_WRITE32(pAd, AMPDU_MAX_LEN_20M1S_MCS8_9, 0xAA);
		}
		else
		{
		}
