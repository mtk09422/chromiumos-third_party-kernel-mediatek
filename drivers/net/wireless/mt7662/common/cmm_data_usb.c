	...
	OUT	USHORT *freeCnt)
{
	...
	{
		rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT)(USBDMApktLen), FALSE, FIFO_HCCA, FALSE /*NextValid*/,  FALSE);
	}
	else
	...
	OUT	USHORT *freeCnt)
{
	...
	if(Status == NDIS_STATUS_SUCCESS)
	{
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bTdlsEntry) &&
			(pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.IamInOffChannel))
		{
			rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT)(dma_len), FALSE, FIFO_HCCA, FALSE /*NextValid*/,  FALSE);
		}
		else
#endif /* CFG_TDLS_SUPPORT */
		{
	...
	OUT USHORT *freeCnt)
{
	...
	{	
		...
		if (Status == NDIS_STATUS_SUCCESS)
		{
			...
			if (QueIdx == QID_HCCA)  //actuall only p2p will use this!
			{
				rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT)(pTxBlk->Priv), 
										FALSE, FIFO_EDCA2, FALSE, FALSE); //zero modify 20120807
			}
			else
#endif /* CONFIG_MULTI_CHANNEL */
			...
			if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bTdlsEntry) &&
				(pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.IamInOffChannel))
			{
				rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT)(pTxBlk->Priv), FALSE, FIFO_HCCA, FALSE /*NextValid*/,  FALSE);
			}
			else
#endif /* CFG_TDLS_SUPPORT */
			{
...
#ifdef CONFIG_MULTI_CHANNEL
	if ((pAd->Mlme.bStartMcc == TRUE) && /*RTMP_CFG80211_VIF_P2P_GO_ON(pAd) && */(QueIdx == QID_HCCA))
	{
		rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT)(BulkOutSize - TXINFO_SIZE), TRUE, FIFO_EDCA2, FALSE,  FALSE);
	}
	else
#endif /* CONFIG_MULTI_CHANNEL */
		rlt_usb_write_txinfo(pAd, pTxInfo, (USHORT)(BulkOutSize - TXINFO_SIZE), TRUE, FIFO_MGMT, FALSE,  FALSE);
...
if (0) {
	...
	dump_txwi(pAd, (TXWI_STRUC *)(pSrcBufVA + TXINFO_SIZE));
}
#endif
//---Add by shiang for debug
