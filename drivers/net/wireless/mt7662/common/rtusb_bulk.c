...
{	
	...
	if ((pAd->PendingRx >= SUBMIT_URB_NUM) || (pRxContext->Readable == TRUE) || (pRxContext->InUse == TRUE) || (pRxContext->bRxHandling == TRUE))
	{
...
VOID BulkInCmdRspEvent(RTMP_ADAPTER *pAd)
{
	...
	if ((pCmdRspEventContext->IRPPending == TRUE)|| (pCmdRspEventContext->Readable == TRUE)|| (pCmdRspEventContext->InUse == TRUE))
	{
...
#ifdef MULTI_URB
			#if 0
			for(idx=1; idx < RX_RING_SIZE; idx++) // check the rest urb if readable or not, and move read index to the readable one.
			{
				pRxContext = &(pAd->RxContext[(pAd->NextRxBulkInReadIndex+idx)%RX_RING_SIZE]);
				if((pRxContext->InUse == FALSE) && (pRxContext->Readable == TRUE) && (pRxContext->bRxHandling == FALSE))
				{
					pAd->NextRxBulkInReadIndex += idx;
					pAd->NextRxBulkInReadIndex %= RX_RING_SIZE;
					break;
				}
			}
			#endif
	...
	else	 /* STATUS_OTHER */
	{
									...
									fRTMP_ADAPTER_NIC_NOT_EXIST))))
		{
			...
			DBGPRINT_RAW(RT_DEBUG_ERROR, ("Bulk In Failed. Status=%d, BIIdx=0x%x, BIRIdx=0x%x, actual_length= 0x%x\n", 
							BulkInStatus, pAd->NextRxBulkInIndex, pAd->NextRxBulkInReadIndex, RTMP_USB_URB_LEN_GET(pRxContext->pUrb))); /*->actual_length)); */
