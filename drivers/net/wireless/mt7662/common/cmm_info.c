	...
	IN	PSTRING			arg)
{
    ...
    )
    {
        ...
        {
            DBGPRINT(RT_DEBUG_TRACE, ("Set_CountryRegion_Proc:: IF is DOWN, Invalid region(2G) idx=%d (max=%d)!\n",
        ...
        DBGPRINT(RT_DEBUG_TRACE, ("Set_CountryRegion_Proc:: IF is DOWN, CountryRegion for 2G band = %d\n", CountryRegion_SetDuringIfDown));
        return TRUE;
    }
	...
	IN	PSTRING			arg)
{
	...
	return -EOPNOTSUPP;
        ...
        )
        {
            ...
            {
                DBGPRINT(RT_DEBUG_TRACE, ("Set_CountryRegion_Proc:: IF is DOWN, Invalid region(5G) idx=%d (max=%d)!\n",
                                         CountryRegionABand_SetDuringIfDown, Country_Region_GroupNum_5GHZ));
                CountryRegionABand_SetDuringIfDownFlag = 0;
                CountryRegionABand_SetDuringIfDown = 0;
            }
            DBGPRINT(RT_DEBUG_TRACE, ("Set_CountryRegion_Proc:: IF is DOWN, CountryRegion for 5G band = %d\n", CountryRegionABand_SetDuringIfDown));
            return TRUE;
        }
#endif /* DYNAMIC_PWR_TABLE_MAPPED_BY_COUNTRY_REGION */
...
static INT	Set_Cmm_WirelessMode_Proc(
	IN RTMP_ADAPTER *pAd,
	IN PSTRING arg,
...
#endif /* CONFIG_STA_SUPPORT */
static VOID	RTMPCommSiteSurveyData(
	IN  PSTRING msg,
	IN  BSS_ENTRY *pBss,
	IN  UINT32 MsgLen)
{
	...
	IN	UINT32 		arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("CustomizedSupportFlag: %d\n", pAd->cfg80211_ctrl.CustomizedSupportFlag));
...
static INT	Show_WirelessMode_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_TxPreamble_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_BGProtection_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_RTSThreshold_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_FragThreshold_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_HtMpduDensity_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_HtBaWinSize_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_CountryRegion_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_CountryRegionABand_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_CountryCode_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
#ifdef AGGREGATION_SUPPORT
static INT	Show_PktAggregate_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_WmmCapable_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_IEEE80211H_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
#ifdef CONFIG_STA_SUPPORT
static INT	Show_NetworkType_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
#ifdef WSC_STA_SUPPORT
static INT	Show_WpsPbcBand_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_AutoReconnect_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_AutoProvisionEn_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_ChangeMode_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_EncrypType_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_DefaultKeyID_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
static INT	Show_STA_RAInfo_Proc(
	IN	PRTMP_ADAPTER	pAd,
	OUT	PSTRING			pBuf,
	IN	ULONG			BufLen)
{
...
INT show_stainfo_proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	for (i = 0, token = rstrtok(arg, &sep[0]); token; token = rstrtok(NULL, &sep[0]), i++)
	{
		DBGPRINT(RT_DEBUG_OFF, ("%s(): token(len=%d) =%s\n",
					__FUNCTION__, (INT32)strlen(token), token));
		if((strlen(token) != 2) || (!isxdigit(*token)) || (!isxdigit(*(token+1))))
			return FALSE;
			...
			switch(phy)
			{
				...
				case MODE_CCK:
					if (mcs < SINGLE_SKU_TABLE_CCK_LENGTH)
					{
						...
						rate_pwr = ad->chipCap.rate_pwr_table.CCK[mcs].mcs_pwr + DefaultTargetPwr + ch_delta_pwr;

						DBGPRINT(RT_DEBUG_TRACE, ("CCK[%d].MCS_Power = %d, DefaultTargetPwr = %d, ch_delta_pwr = %d\n",
						mcs, ad->chipCap.rate_pwr_table.CCK[mcs].mcs_pwr, DefaultTargetPwr, ch_delta_pwr));
						sprintf(extra+strlen(extra),"rate_pwr=%lu(%lu.%d dBm)",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5);
						DBGPRINT(RT_DEBUG_ERROR, ("rate_pwr=%lu(%lu.%d dBm)\n",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5));
				...
				case MODE_OFDM:
					if (mcs < SINGLE_SKU_TABLE_OFDM_LENGTH)
					{
						...
						mcs = ((mcs%2) == 0) ? mcs : ((mcs == 7)? mcs : (mcs - 1));

						rate_pwr = ad->chipCap.rate_pwr_table.OFDM[mcs].mcs_pwr + DefaultTargetPwr + ch_delta_pwr;
						DBGPRINT(RT_DEBUG_TRACE, ("OFDM[%d].MCS_Power = %d, DefaultTargetPwr = %d, ch_delta_pwr = %d\n",
						mcs, ad->chipCap.rate_pwr_table.OFDM[mcs].mcs_pwr, DefaultTargetPwr, ch_delta_pwr));
						sprintf(extra+strlen(extra),"rate_pwr=%lu(%lu.%d dBm)",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5);
						DBGPRINT(RT_DEBUG_ERROR, ("rate_pwr=%lu(%lu.%d dBm)\n",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5));
					}
					break;
              ...
              /* HT/VHT1SS MCS 4/5 use HT[4], HT/VHT1SS MCS 6 use HT[6] */
							...
							if (mcs <= 7)
								...
								mcs = ((mcs%2) == 0) ? mcs : ((mcs == 15)? mcs : (mcs - 1));

							rate_pwr = ad->chipCap.rate_pwr_table.HT[mcs].mcs_pwr + DefaultTargetPwr + ch_delta_pwr;
							DBGPRINT(RT_DEBUG_TRACE, ("@HT[%d].MCS_Power = %d, DefaultTargetPwr = %d, ch_delta_pwr = %d\n",
							mcs, ad->chipCap.rate_pwr_table.HT[mcs].mcs_pwr, DefaultTargetPwr, ch_delta_pwr));
							sprintf(extra+strlen(extra),"rate_pwr=%lu(%lu.%d dBm)",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5);
							DBGPRINT(RT_DEBUG_ERROR,("rate_pwr=%lu(%lu.%d dBm)\n",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5));
				...
				case MODE_VHT:
					if (mcs < SINGLE_SKU_TABLE_VHT_LENGTH)
					{
							...
							if (ss == 1)
							{
								...
								{
									rate_pwr = ad->chipCap.rate_pwr_table.VHT1SS[mcs].mcs_pwr + DefaultTargetPwr + ch_delta_pwr;
									DBGPRINT(RT_DEBUG_TRACE, ("VHT1SS[%d].MCS_Power = %d, DefaultTargetPwr = %d, ch_delta_pwr = %d\n",
											mcs, ad->chipCap.rate_pwr_table.VHT1SS[mcs].mcs_pwr, DefaultTargetPwr, ch_delta_pwr));
								}
							}
							else if (ss == 2)
							{
								...
								{
									rate_pwr = ad->chipCap.rate_pwr_table.VHT2SS[mcs].mcs_pwr + DefaultTargetPwr + ch_delta_pwr;
									DBGPRINT(RT_DEBUG_TRACE, ("VHT2SS[%d].MCS_Power = %d, DefaultTargetPwr = %d, ch_delta_pwr = %d\n",
										mcs, ad->chipCap.rate_pwr_table.VHT2SS[mcs].mcs_pwr, DefaultTargetPwr, ch_delta_pwr));
								}
							}
							sprintf(extra+strlen(extra),"rate_pwr=%lu(%lu.%d dBm)",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5);
							DBGPRINT(RT_DEBUG_ERROR, ("rate_pwr=%lu(%lu.%d dBm)\n",rate_pwr,(rate_pwr/2),(rate_pwr%2==0)?0:5));
...
#ifdef CUSTOMIZED_COUNTRY_REGION_CE_1
    {
        ...
        for (idx=0; idx < REGION_MAXIMUM_BG_BAND+1; idx++)
        {
            if (pAd->CommonCfg.CountryRegion == CountryRegion_PowerTab_Mapping[idx][0])
            {
                ...
                sprintf(path, "%s_%d.dat", DYNAMIC_SINGLE_SKU_TABLE_FILE_NAME_PREFIX, CountryRegion_PowerTab_Mapping[idx][1]);
                DBGPRINT(RT_DEBUG_TRACE, ("Found matched dynamic power table=[%s]!\n", path));
                return 0;
            }
        }
    }
	...
	IN UINT32 size)
{
	...
	if (!WMODE_5G_ONLY(pAd->CommonCfg.PhyMode))
	{
        ...
        for (i = 0; i < TotalChNum(pChDesc2G); i++)
        {
			sprintf(pOutBuf, "%s\t\tChannel: %d \tStatus: %s    MaxBw: %s\n",
				...
				(GetChannelFlag(pChDesc2G, i) == CHANNEL_DISABLED?"Disable":(GetChannelFlag(pChDesc2G, i) == CHANNEL_DEFAULT_PROP?"Active":"Passive")),
#ifdef CUSTOMIZED_BW_SETTING
				(RTMP_GetCustomizedChannelBw(pAd, GetChannel_2GHZ(pChDesc2G, i))==BW_80?"VHT80":(RTMP_GetCustomizedChannelBw(pAd, GetChannel_2GHZ(pChDesc2G, i))==BW_40?"HT40":"HT20"))
#else
				"None"
	...
	if (WMODE_CAP_5G(pAd->CommonCfg.PhyMode))
	{
        ...
        for (i = 0; i < TotalChNum(pChDesc5G); i++)
        {
				...
				(GetChannelFlag(pChDesc5G, i) == CHANNEL_DISABLED?"Disable":(GetChannelFlag(pChDesc5G, i) == CHANNEL_DEFAULT_PROP?"Active":"Passive")),
#ifdef CUSTOMIZED_BW_SETTING
				(RTMP_GetCustomizedChannelBw(pAd, GetChannel_5GHZ(pChDesc5G, i))==BW_80?"VHT80":(RTMP_GetCustomizedChannelBw(pAd, GetChannel_5GHZ(pChDesc5G, i))==BW_40?"HT40":"HT20"))
#else
				"None"
#endif /* CUSTOMIZED_BW_SETTING */
				);
