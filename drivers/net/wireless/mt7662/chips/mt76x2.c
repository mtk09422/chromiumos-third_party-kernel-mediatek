...
//It should be common
#if 0//defined(RT_CFG80211_SUPPORT) && defined(RT_CFG80211_P2P_SUPPORT)
    if (RTMP_CFG80211_VIF_P2P_GO_ON(ad))
    {
        /* When as GO mode, if scan is requested, sometimes we could see "Wait until MAC 0x1200 bit0 .... > 300 times". */
        /* Howie/Ben : [IBI reset+ BBP reset ] to prevent MAC stop incompletely and causing BBP TX busy luck */
    	RTMP_BBP_IO_READ32(ad, CORE_R4, &value);
        value |= 0x3;
        RTMP_BBP_IO_WRITE32(ad, CORE_R4, value);
        value &= (~0x3);
        RTMP_BBP_IO_WRITE32(ad, CORE_R4, value);
    }
#endif /* defined(RT_CFG80211_SUPPORT) && defined(RT_CFG80211_P2P_SUPPORT) */
...
#if 0 /* qqq empty funtion for 76x2 */
...
static void periodic_monitor_false_cca_adjust_init_vga(RTMP_ADAPTER *pAd, UCHAR *initvga_0, UCHAR *initvga_1)
{
#ifdef MT76x2
    UCHAR lowbound_0 = (pAd->CommonCfg.lna_vga_ctl.agc_vga_init_0 >= 0x10) ? (pAd->CommonCfg.lna_vga_ctl.agc_vga_init_0 - 0x10):(pAd->CommonCfg.lna_vga_ctl.agc_vga_init_0);
    UCHAR lowbound_1 = (pAd->CommonCfg.lna_vga_ctl.agc_vga_init_1 >= 0x10) ? (pAd->CommonCfg.lna_vga_ctl.agc_vga_init_1 - 0x10):(pAd->CommonCfg.lna_vga_ctl.agc_vga_init_1);
    ...
    if (pAd->CommonCfg.Channel<=48 || pAd->CommonCfg.Channel>=149)
    {
        ...
        {
            (*initvga_0) = (((*initvga_0)-(0x2)) <= lowbound_0)?(lowbound_0):((*initvga_0)-(0x2));
            (*initvga_1) = (((*initvga_1)-(0x2)) <= lowbound_1)?(lowbound_1):((*initvga_1)-(0x2));
        }
        else if (fcca_1s_count < 10)
        {
            (*initvga_0) = (((*initvga_0)+(0x2)) >= upbound_0)?(upbound_0):((*initvga_0)+(0x2));
            (*initvga_1) = (((*initvga_1)+(0x2)) >= upbound_1)?(upbound_1):((*initvga_1)+(0x2));
        }
        else
        {
            /* No change */
...
static void periodic_monitor_rssi_adjust_vga_ap_mode(RTMP_ADAPTER *pAd)
{
    ...
    static int there_was_client = 0; /* remember previous time has client or not */
...
static void host_periodic_monitor_rssi_adjust_vga(RTMP_ADAPTER *pAd)
{
	...
	if (!pAd->CommonCfg.DynamicVGA_Disabled && RTMP_CFG80211_VIF_P2P_GO_ON(pAd) && !INFRA_ON(pAd)) {
#else
...
void periodic_monitor_rssi_adjust_vga(RTMP_ADAPTER *pAd)
{
	...
	static INT32 previous_rssi_in_75_range = 0;
	RTMP_BBP_IO_READ32(pAd, AGC1_R8, &bbp_val1);
	RTMP_BBP_IO_READ32(pAd, AGC1_R9, &bbp_val2);
    if (pAd->CommonCfg.lna_vga_ctl.agc_vga_init_0 == 0 || pAd->CommonCfg.lna_vga_ctl.agc_vga_init_1 == 0)
        DBGPRINT(RT_DEBUG_ERROR, ("Initial VGA gain is ZERO! (init_0=0x%x, init_1=0x%x)\n",
...
#if 0
#ifdef CONFIG_AP_SUPPORT
			if (pAd->Mlme.OneSecPeriodicRound % (pAd->chipCap.dynamic_lna_trigger_timer + 1) == 0) {
				if (pAd->chipCap.microwave_enable == TRUE) {
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R62, 0x02E7CEB0); /* disable microwave func */
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R35, pAd->chipCap.agc1_r35_backup);
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R39, pAd->chipCap.agc1_r39_backup);
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R41, pAd->chipCap.agc1_r41_backup);
					pAd->chipCap.microwave_enable = FALSE;
					DBGPRINT(RT_DEBUG_INFO, ("%s::disable microwave func\n", __FUNCTION__));
				} else
					DBGPRINT(RT_DEBUG_INFO, ("%s::do nothing for dynamic LNA#1\n", __FUNCTION__));
			} else {
				if (pAd->RalinkCounters.OneSecFalseCCACnt > pAd->CommonCfg.lna_vga_ctl.nFalseCCATh) {
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R62, 0x83E7CEB0); /* enable microwave func */
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R35, 0x24242424);
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R39, 0x2A2A3036);
					RTMP_BBP_IO_WRITE32(pAd, AGC1_R41, 0x38383E45);
					pAd->chipCap.microwave_enable = TRUE;
					DBGPRINT(RT_DEBUG_INFO, ("%s::enable microwave func\n", __FUNCTION__));
				} else
					DBGPRINT(RT_DEBUG_INFO, ("%s::do nothing for dynamic LNA#2\n", __FUNCTION__));
			}
#endif /* CONFIG_AP_SUPPORT */
#endif /* 0 */
		}
...
#ifdef WCX_SUPPORT
	.fw_bin_file_name = "WIFI_RAM_CODE_MT76x2.bin",
#else
...
#ifdef WCX_SUPPORT
	.rom_patch_bin_file_name = "mt76x2_patch_hdr.bin",
#else
