...
loadfw_protect:
	...
	if (cap->load_code_method == BIN_FILE_METHOD) {
		DBGPRINT(RT_DEBUG_OFF, ("Load rom patch from BinFile $FIRMWARE_PATH/%s\n", cap->rom_patch_bin_file_name));
	...
	if (!cap->rom_patch) {
		if (cap->load_code_method == BIN_FILE_METHOD) {
			...
			DBGPRINT(RT_DEBUG_ERROR, ("%s: Load rom_patch from BinFile failed!!!\n", __FUNCTION__));
			DBGPRINT(RT_DEBUG_ERROR, ("Please check if fw(%s) exist in firmware path such as "
									  "\"/etc/firmware\" or \"/lib/modules\"\n", cap->rom_patch_bin_file_name));
			DBGPRINT(RT_DEBUG_ERROR, ("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n"));
		} else {
...
loadfw_protect:
...
#ifndef BB_SOC
	if (cap->load_code_method == BIN_FILE_METHOD) {
		DBGPRINT(RT_DEBUG_OFF, ("Load WiFi firmware from BinFile $FIRMWARE_PATH/%s\n", cap->fw_bin_file_name));
	...
	if (!cap->FWImageName) {
		if (cap->load_code_method == BIN_FILE_METHOD) {
			DBGPRINT(RT_DEBUG_ERROR, ("\nxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n"));
			DBGPRINT(RT_DEBUG_ERROR, ("%s: Load firmware from BinFile failed!!!\n", __FUNCTION__));
			DBGPRINT(RT_DEBUG_ERROR, ("Please check if fw(%s) exist in firmware path such as "
									  "\"/etc/firmware\" or \"/lib/modules\"\n", cap->fw_bin_file_name));
			DBGPRINT(RT_DEBUG_ERROR, ("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n"));
		} else {
...
load_patch_protect:
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s --> ChipID : 0x%4x\n", __FUNCTION__, ad->ChipID));

	/* Check rom patch if ready */
	if (MT_REV_GTE(ad, MT76x2, REV_MT76x2E3)) {
		RTUSBReadMACRegister(ad, CLOCK_CTL, &mac_value);
		DBGPRINT(RT_DEBUG_OFF, ("%s --> mac_value : 0x%4x , cap->rom_code_protect = %u\n", __FUNCTION__,
							mac_value,
	...
	if (cap->load_code_method == BIN_FILE_METHOD) {
		DBGPRINT(RT_DEBUG_OFF, ("Load rom_patch from BinFile $FIRMWARE_PATH/%s\n", cap->rom_patch_bin_file_name));
...
static void andes_rx_process_cmd_msg(RTMP_ADAPTER *ad, struct cmd_msg *rx_msg)
{
	...
	{
		DBGPRINT(RT_DEBUG_WARN, ("RX RSP_EVT_TYPE_TDLS_BACK_TO_BASE_CHANNEL\n"));

		//start base timer //start when we got DTIM
		//BOOLEAN TimerCancelled;
		//RTMPCancelTimer(&ad->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelSwitchTimer, &TimerCancelled);
		//RTMPSetTimer(&ad->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BaseChannelSwitchTimer, 15);
		ad->StaCfg.wpa_supplicant_info.CFG_Tdls_info.IamInOffChannel = FALSE;
	}
	if(rx_info->evt_type == RSP_EVT_TYPE_TDLS_STAY_TIME_OUT)
	{
		DBGPRINT(RT_DEBUG_WARN, ("RX RSP_EVT_TYPE_TDLS_STAY_TIME_OUT\n"));
		PCFG_TDLS_STRUCT pCfgTdls = &ad->StaCfg.wpa_supplicant_info.CFG_Tdls_info;
		cfg_tdls_chsw_resp(ad,pCfgTdls->CHSWPeerMacAddr,pCfgTdls->ChSwitchTime,pCfgTdls->ChSwitchTimeout,0);
	}
	...
	if (rx_info->evt_type == RSP_EVT_TYPE_COEX_RSP_EVENT)
	{
		...
		pRxRspEvtPayload=GET_OS_PKT_DATAPTR(net_pkt) + sizeof(*rx_info);
		DBGPRINT(RT_DEBUG_TRACE, ("%s: RSP_EVT_TYPE_COEX_RSP_EVENT, RspEvtType = %d (len=%d)\n",
					__FUNCTION__,
					rx_info->evt_type, rx_info->pkt_len));

		pDW = (PULONG)(&pRxRspEvtPayload[0]);
		DBGPRINT(RT_DEBUG_TRACE, ("%s: 0x%lu\n",
			__FUNCTION__,
			*pDW));
        ...
        switch (*pDW)
        {
            ...
            case RSP_UpdateCOEXMode:
                ...
                DBGPRINT(RT_DEBUG_TRACE, ("Receive COEX update mode %lu\n",*pDW));
            ...
            case RSP_BT_DEVICE_LINK_STATUS:
                DBGPRINT(RT_DEBUG_TRACE, ("Receive COEX OPERATION RSP_BT_DEVICE_LINK_STATUS\n"));
                pDW = (PULONG)(&pRxRspEvtPayload[4]);
                ad->BTStatusFlags = (*pDW);
                CoexParseBTStatus(ad);
                DBGPRINT(RT_DEBUG_TRACE, ("Receive COEX BT_DEVICE_LINK_STATUS = %lu\n",*pDW));


            break;

            default:
                DBGPRINT(RT_DEBUG_ERROR, ("Receive COEX update packets (%lu) BUT is a unkonw COEX Operation\n", *pDW));
                break;
...
static void usb_kick_out_cmd_msg_complete(PURB urb)
{
	...
	if (msg->need_rsp)
		andes_queue_tail_cmd_msg(&ctl->ackq, msg, wait_ack);
	else
		andes_queue_tail_cmd_msg(&ctl->tx_doneq, msg, tx_done);
...
retransmit:
	...
	if (need_wait) {
		...
		if (!andes_wait_for_complete_timeout(msg, msg->timeout)) {
			ret = NDIS_STATUS_FAILURE;
			DBGPRINT(RT_DEBUG_ERROR, ("command %d timeout %d ms\n", msg->type, msg->timeout));
...
#ifdef NEW_WOW_SUPPORT
int andes_wow_enable(struct _RTMP_ADAPTER *ad)// , RTMP_ADAPTER *ad, u32 cal_id, u32 param)
{
	...
	printk("\x1B[34mF/W Heart beat(0x738): 0x%x\x1b[m\n", value);
	...
	*/
	...
	{
	printk("\x1b[31mSet TCP packet wake up\x1b[m\n");
	...
	for(i=0;i<4;i++)
		...
		tcp_cfg.TcpPortCountV4 = ad->WOW_Cfg.TcpPortCntV4;
		if(ad->WOW_Cfg.TcpPortCntV4!=0)
	{
			for(i=0;i<tcp_cfg.TcpPortCountV4;i++)
	{
				tcp_cfg.TcpPortV4[i] = ad->WOW_Cfg.TcpPortV4[i];
				printk("\x1b[31mSet TCP port v4:%d\x1b[m\n",tcp_cfg.TcpPortV4[i]);
			}
		}

		tcp_cfg.TcpPortCountV6 = ad->WOW_Cfg.TcpPortCntV6;
		if(ad->WOW_Cfg.TcpPortCntV6!=0)
		{
			for(i=0;i<tcp_cfg.TcpPortCountV6;i++)
			{
				tcp_cfg.TcpPortV6[i] = ad->WOW_Cfg.TcpPortV6[i];
				printk("\x1b[31mSet TCP port v6:%d\x1b[m\n",tcp_cfg.TcpPortV6[i]);
			}
	}
	...
	if (ret) {
		...
		printk("\x1b[31mTCP IP:%d.%d.%d.%d\nTCP port count v4:%d\nTCP port count v6:%d\x1b[m\n",
	...
	{
	printk("\x1b[31mSet UDP packet wake up\x1b[m\n");
	var_len = sizeof(udp_cfg);
	NdisZeroMemory(&udp_cfg, var_len);

	udp_cfg.Config_Type = WOW_UDP_CFG;

		udp_cfg.UdpPortCountV4= ad->WOW_Cfg.UdpPortCntV4;
		if(ad->WOW_Cfg.UdpPortCntV4!=0)
	{
			for(i=0;i<udp_cfg.UdpPortCountV4;i++)
	{
				udp_cfg.UdpPortV4[i] = ad->WOW_Cfg.UdpPortV4[i];
				printk("\x1b[31mSet UDP port v4:%d\x1b[m\n",udp_cfg.UdpPortV4[i]);
			}
		}

		udp_cfg.UdpPortCountV6= ad->WOW_Cfg.UdpPortCntV6;
		if(ad->WOW_Cfg.UdpPortCntV6!=0)
		{
			for(i=0;i<udp_cfg.UdpPortCountV6;i++)
			{
				udp_cfg.UdpPortV6[i] = ad->WOW_Cfg.UdpPortV6[i];
				printk("\x1b[31mSet UDP port v6:%d\x1b[m\n",udp_cfg.UdpPortV6[i]);
			}
	}
	...
	if (ret) {
		...
		printk("\x1b[31mUDP port count v4:%d\nUDP port count v6:%d\x1b[m\n", udp_cfg.UdpPortCountV4, udp_cfg.UdpPortCountV6);
	RtmpOsMsDelay(1);
	}
	...
	printk("\x1b[31mARP IP:%d.%d.%d.%d\x1b[m\n",
			offload_arp_cfg.IP[0], offload_arp_cfg.IP[1], offload_arp_cfg.IP[2], offload_arp_cfg.IP[3]);
	RtmpOsMsDelay(1);
	...
	else
	{
		...
		{
			wow_param.GPIO_Action_1 = ((ad->WOW_Cfg.bHighActive << 31) | 0);
			wow_param.GPIO_Action_2 = ((ad->WOW_Cfg.bHighActive << 31) | ad->WOW_Cfg.nHoldTime);
	...
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m aid =  %d\x1b[m\n", wow_aid_channel.Aid));
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m central channel =  %d\x1b[m\n", wow_aid_channel.Channel));
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m extension channel setting =  %d\x1b[m\n", wow_aid_channel.extCH));
	...
	ad->WOW_Cfg.bInSuspendMode = WOW_SUSPEND_COMPLETE;
	DBGPRINT(RT_DEBUG_TRACE, ("\x1b[31m  bInSuspendMode=  %d\x1b[m\n", ad->WOW_Cfg.bInSuspendMode));
...
int andes_wow_disable(struct _RTMP_ADAPTER *ad)//RTMP_ADAPTER *ad, u32 cal_id, u32 param)
{
	NEW_WOW_PARAM_STRUCT wow_param;
	struct cmd_msg *msg;
	unsigned int var_len;
	...
	RTUSBBulkReceive(ad);
//	RTUSBBulkCmdRspEventReceive(ad);
	usb_rx_cmd_msgs_receive(ad);
...
u32 start_tsf)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[31m @@@@@ %s:channel(%u,%u), bw(%u,%u), role(%u,%u), cycle_time(%u,%u), wait_time(%u), null_cnt(%u), tssi_slope(0x%04x,%04x), start_tsf(0x%08x) \x1b[m\n",
		__FUNCTION__, channel_1st, channel_2nd, bw_1st, bw_2nd, role_1st, role_2nd, cycle_time_1st, cycle_time_2nd, wait_time, null_cnt, tssi_slope_offset_1st, tssi_slope_offset_2nd, start_tsf));
...
int andes_mcc_stop(struct _RTMP_ADAPTER *ad, u8 parking_channel, u8 auto_resume_mode, u16 auto_resume_time, u32 auto_resume_tsf)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("\x1b[32m @@@@@ %s:parking_channel(%u), auto_resume_mode(%u), auto_resume_time(0x%04x),auto_resume_tsf(0x%08x) \x1b[m\n",
		__FUNCTION__, parking_channel, auto_resume_mode, auto_resume_time,auto_resume_tsf));
