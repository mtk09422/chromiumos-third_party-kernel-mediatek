...
static VOID SendAndesCoexFrameInfo(
	...
	IN ULONG TriggerNumber)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s: Triggernumber = %lu, Valid = %lu, NodeType = %lu, BssHashID = %lu, "
							  "FrameType = %lu, CmdParametersLength = %d\n",
					__FUNCTION__,
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	for (iter=0; iter < NULLFRAMESPACE; iter++ )
	{
		...
		{
	 	   DBGPRINT(RT_DEBUG_TRACE, ("Coex: Send protection frame %lu\n", iter));
	...
	IN ULONG					 BssHashID)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: LinkStatus = %lu, BW = %lu, Channel = %lu, "
			 "BssHashID = %lu PktLength = %d\n",
			__FUNCTION__,
			coexAFH.LinkStatus,
			coexAFH.BW,
			coexAFH.Channel,
...
static VOID FillProtectionFrameSpace(
	IN	PRTMP_ADAPTER	pAd,
	IN	ULONG	Triggernumber,
	IN	  ULONG	Valid,
	IN	  ULONG	NodeType,
...
static VOID PrepareProtectionFrame(
	IN	PRTMP_ADAPTER	pAd,
	IN	ULONG		   Type,
	IN	ULONG		   Number,
	IN	ULONG		NAV,
	...
	IN	PMAC_TABLE_ENTRY pEntry)
{
	...
	//write_tmac_info(pAd, (UCHAR *)pTxWI, FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, 0, BSSID_WCID, Length,
	//					0, 0, (UCHAR)pAd->CommonCfg.MlmeTransmit.field.MCS, IFS_HTTXOP, &pAd->CommonCfg.MlmeTransmit);
	RTMPWriteTxWI(pAd,
					pTxWI,
					FALSE,
					FALSE,
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Type =%lu \n",Type));
...
static UCHAR CheckAvailableNullFrameSpace(
	IN PRTMP_ADAPTER	pAd)
{
	UCHAR iter = 0;
	for (iter=0; iter < NULLFRAMESPACE; iter++ )
	...
	)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: CoexOperation = %lu, WlanStatus = %lu, PrivilegeTime = %lu,"
			 " BssHashID = %lu, PktLength = %d\n",
		__FUNCTION__,
	...
	)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s: CoexOperation = %lu, CoexMode = %lu\n, PktLength = %d\n",
		__FUNCTION__,
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_MT7650(pAd)  || IS_MT7630(pAd) || IS_MT76x2(pAd))
	{
		...
		DBGPRINT(RT_DEBUG_ERROR, ("==>InitBTCoexistence CoexMode.DefaultMode=%s(%lu)\n",
...
VOID MLMEHook(
	...
	DBGPRINT(RT_DEBUG_TRACE, ("COEX: CoexWLANPrivilegeTime = %lu\n",  pAd->CommonCfg.CoexWLANPrivilegeTime));
	DBGPRINT(RT_DEBUG_TRACE, ("COEX: unit = %lu\n",  unit));
		...
		)
{
	...
	if (IS_MT76XXBTCOMBO(pAd) && (BT_STATUS_TEST_FLAG(pAd,fBTSTATUS_BT_ACTIVE)))
	{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR,("!!COEX:Strange CoexMode = %lu\n",pAd->CoexMode.CurrentMode));
		}
	}
...
{
	if (pAd->CoexMode.CurrentMode == COEX_MODE_TDD)
	{
	...
	else if (pAd->CoexMode.CurrentMode == COEX_MODE_FDD)
	{
		...
		(pAd->CoexMode.CurrentMode == COEX_MODE_FDD))
	{
		...
		if (pAd->CoexMode.FDD_CRUpdated)
		{
			...
			if (!BT_STATUS_TEST_LINK_UP(pAd))
			{
				...
				{
					DBGPRINT(RT_DEBUG_ERROR, ("* BT is inactive, restore coex RF and PD CR settings\n"));
					DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, restore rf0_24c to 0x%08x\n",
							__FUNCTION__, __LINE__, pAd->CoexMode.RF0NarrAnaFiltBW_OrigVal));
					mt_rf_write(pAd, 0, 0x24C, (u32)pAd->CoexMode.RF0NarrAnaFiltBW_OrigVal);
					pAd->CoexMode.RF0NarrAnaFiltBW_OrigVal = 0;
				}
				if (pAd->CoexMode.RF1NarrAnaFiltBW_OrigVal != 0)
				{
					DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, restore rf1_24c to 0x%08x\n",
							__FUNCTION__, __LINE__, pAd->CoexMode.RF1NarrAnaFiltBW_OrigVal));
					mt_rf_write(pAd, 1, 0x24C, (u32)pAd->CoexMode.RF1NarrAnaFiltBW_OrigVal);
				...
				if (pAd->CoexMode.PDThreashold_OrigVal != 0)
				{
					DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, restore PD to 0x%02x\n",
							__FUNCTION__, __LINE__, pAd->CoexMode.PDThreashold_OrigVal));
					RTMP_BBP_IO_READ32(pAd, AGC1_R35, &val);
					val = ((val & 0x00ffffff) | ((pAd->CoexMode.PDThreashold_OrigVal) << 24));
		...
		else
		{
			...
			if (BT_STATUS_TEST_LINK_UP(pAd))
			{
				DBGPRINT(RT_DEBUG_ERROR, ("* BT is active, apply coex RF and PD CR settings\n"));
				/****** (1) Narrowest Analog Filter BW(RF 0x24c=0x3F7F7F7F) ********/
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, original rf0_24c=0x%08x, rf1_24c=0x%08x\n",
					__FUNCTION__, __LINE__, pAd->CoexMode.RF0NarrAnaFiltBW_OrigVal, pAd->CoexMode.RF1NarrAnaFiltBW_OrigVal));
				...
				mt_rf_read(pAd, 0, 0x24C, &val);
				DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, new rf0_24c=0x%08x\n", __FUNCTION__, __LINE__, val));
				mt_rf_read(pAd, 1, 0x24C, &val);
				DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, new rf1_24c=0x%08x\n", __FUNCTION__, __LINE__, val));
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, original pd_thres=0x%08x(b[31:24]=0x%x)\n", __FUNCTION__, __LINE__,
						val, pAd->CoexMode.PDThreashold_OrigVal));
				/* replace with new value */
				val = ((val & 0x00ffffff) | (PD_THRESHOLD_COEX << 24));
				RTMP_BBP_IO_WRITE32(pAd, AGC1_R35, val);
				/* read again to confirm */
				RTMP_BBP_IO_READ32(pAd, AGC1_R35, &val);
				DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, new pd_thres=0x%08x\n", __FUNCTION__, __LINE__, val));
...
VOID CoexLinkUpHandler(RTMP_ADAPTER *pAd)
{
    if (IS_MT76XXBTCOMBO(pAd))
    {
...
VOID CoexFDD_HtBwSwitchCheck(RTMP_ADAPTER *pAd)
{
     ...
     */
    static INT32 CoexFddEverForceSwitchBw20 = 0;
    static INT32 CoexFddOriginalBw = BW_20;
    static INT32 CoexFddBTLinkDownCount = 0;
    CHAR TmpStr[8] = "";

    DBGPRINT(RT_DEBUG_TRACE, ("%s --> BW = %d , ch = %d , crl_ch = %d , infra_on = %d , BT_STATUS_TEST_LINK_UP = %d\n",
						__FUNCTION__,
						pAd->CommonCfg.RegTransmitSetting.field.BW,
						pAd->CommonCfg.Channel,
						pAd->CommonCfg.CentralChannel,
						INFRA_ON(pAd),
						BT_STATUS_TEST_LINK_UP(pAd)));
    ...
    if (BT_STATUS_TEST_LINK_UP(pAd))
    {
        ...
        if (CoexFddEverForceSwitchBw20 == 0)
        {
                ...
                (pAd->CommonCfg.Channel != pAd->CommonCfg.CentralChannel))
            {
                ...
                */
                DBGPRINT(RT_DEBUG_ERROR, ("%s: BT Linkup and currently WiFi associated on 2.4G with BW40 mode, force link down\n",
                                            __FUNCTION__));
                LinkDown(pAd, FALSE);
                DBGPRINT(RT_DEBUG_ERROR, ("%s: FDD WiFi at 2.4G band with non-BW20 mode, switching to BW20 mode...\n",
                                            __FUNCTION__));
                Set_HtBw_Proc(pAd, "0");
            }
        }
        ...
        if (CoexFddEverForceSwitchBw20)
        {
            if (!INFRA_ON(pAd))
            {
                if (++CoexFddBTLinkDownCount >= 30)
                {
                    ...
                    DBGPRINT(RT_DEBUG_ERROR, ("%s: BT link is off for %d sec, switch back to original mode (BW=%s)...\n",
                        __FUNCTION__, CoexFddBTLinkDownCount, TmpStr));
