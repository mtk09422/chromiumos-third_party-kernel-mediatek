...
static int mt_private_meta_set(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("<== %s(), wrq->length = %d\n", __FUNCTION__,wrq->length));
	GET_PAD_FROM_NET_DEV(pAd, dev);
	RTMP_STA_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_STA_META_SET, 0, extra, IW_PRIV_SIZE_MASK, RT_DEV_PRIV_FLAGS_GET(dev));
	return stat;
}
...
static int mt_private_meta_qurey(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("<== %s(), wrq->length = %d, wrq->pointer:%p\n", __FUNCTION__,wrq->length,wrq->pointer));
	GET_PAD_FROM_NET_DEV(pAd, dev);
	wrqin.u.data.pointer = wrq->pointer;
	wrqin.u.data.length = wrq->length;
	RTMP_STA_IoctlHandle(pAd, &wrqin, CMD_RTPRIV_IOCTL_STA_META_QUERY, 0, extra, IW_PRIV_SIZE_MASK, RT_DEV_PRIV_FLAGS_GET(dev));
