...
static inline BOOLEAN __RtmpOSTaskWait(
	IN VOID *pReserved,
	IN OS_TASK *pTask,
	IN INT32 *pStatus)
{
	...
	IN PSTRING pNamePrefix)
{
	...
	if (status != NDIS_STATUS_SUCCESS) {
		...
		DBGPRINT(RT_DEBUG_OFF, ("Allocate net device ops success! -> %p \n", pNetDev->netdev_ops));
	}
#endif
...
void OS_LOAD_CODE_FROM_BIN(unsigned char **image, char *bin_name, void *inf_dev, UINT32 *code_len)
{
	...
	if (request_firmware(&fw_entry, bin_name, dev) != 0) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: FW not available at $FIRMWARE_PATH/%s\n", __FUNCTION__, bin_name));
		*image = NULL;
		return;
	}
