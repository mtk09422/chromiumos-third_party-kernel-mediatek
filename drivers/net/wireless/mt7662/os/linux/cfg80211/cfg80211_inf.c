...
BOOLEAN CFG80211DRV_OpsChgVirtualInf(RTMP_ADAPTER *pAd, VOID *pData)
{
    ...
    CFG80211DBG(RT_DEBUG_TRACE, ("80211> CFG80211DRV_OpsChgVirtualInf  newType %d  oldType %d\n",oldType,newType));
    if (strcmp(pVifParm->net_dev->name, "p2p0") == 0) {
        switch (newType)
        {
            ...
            case RT_CMD_80211_IFTYPE_P2P_GO:
    			...
    			COPY_MAC_ADDR(pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.if_addr, pAd->cfg80211_ctrl.P2PCurrentAddress);
    			COPY_MAC_ADDR(pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.bssid, pAd->cfg80211_ctrl.P2PCurrentAddress);
                break;
...
static INT CFG80211_VirtualIF_Close(PNET_DEV dev_p)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: ===> %s (iftype=0x%x)\n", __FUNCTION__, RTMP_OS_NETDEV_GET_DEVNAME(dev_p), dev_p->ieee80211_ptr->iftype));
...
#ifdef RT_CFG80211_P2P_STATIC_CONCURRENT_DEVICE
        return; //move to CFG80211DRV_OpsChgVirtualInf
#endif /* RT_CFG80211_P2P_STATIC_CONCURRENT_DEVICE */
	...
	IN  UINT32                DevType)
{
	...
	if (dev_p)
	{
		...
		if (isGoOn)
		{
			...
			{
				DBGPRINT(RT_DEBUG_TRACE, ("GO remove & switch to Infra BW = %d  pAd->StaCfg.wdev.CentralChannel %d \n",pAd->StaCfg.wdev.bw,pAd->StaCfg.wdev.CentralChannel));
...
/*after p2p cli connect , neet to change to default configure*/
		...
		else if (pAd->flg_apcli_init)
		{
			...
			//if (pAd->Mlme.bStartScc == TRUE)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("@@@ GC remove & switch to Infra BW = %d  pAd->StaCfg.wdev.CentralChannel %d \n",pAd->StaCfg.wdev.bw,pAd->StaCfg.wdev.CentralChannel));
				pAd->Mlme.bStartScc = FALSE;
				AsicSwitchChannel(pAd, pAd->StaCfg.wdev.CentralChannel, FALSE);
				AsicLockChannel(pAd, pAd->StaCfg.wdev.CentralChannel);
				bbp_set_bw(pAd, pAd->StaCfg.wdev.bw);
			...
			if (pAd->CommonCfg.Channel != 1)
		{
			...
			CFG80211DBG(RT_DEBUG_ERROR, ("80211> %s, Restore to channel %d\n", __FUNCTION__, pAd->CommonCfg.Channel));
		...
		/* Do not free wireless dev due to NL80211_FLAG_NEED_WDEV is announced by new kernel's VIF remove operation */
