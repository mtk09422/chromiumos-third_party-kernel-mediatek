...
int mt76xx_set_pmk(RTMP_ADAPTER *pAd, const u8 *bssid, const u8 *pmkid)
{
	...
	last = min(cfg->SavedPMKNum, ARRAY_SIZE(cfg->SavedPMK));
	/* find the bssid */
...
int mt76xx_del_pmk(RTMP_ADAPTER *pAd, const u8 *bssid)
{
	...
	--last;
	for (; idx < last; idx++)
		memcpy(&cfg->SavedPMK[idx], &cfg->SavedPMK[idx + 1],
		       sizeof(cfg->SavedPMK[idx]));
	--cfg->SavedPMKNum;
...
========================================================================
*/
static int mt76xx_cfg80211_flush_pmksa(struct wiphy *wiphy,
				       struct net_device *dev)
{
#ifdef CONFIG_STA_SUPPORT
	RTMP_ADAPTER *pAd;
...
#ifdef CONFIG_AP_SUPPORT
static VOID CFG80211_UpdateAssocRespExtraIe(
	VOID *pAdOrg,
	UCHAR *assocresp_ies,
...
static int mt76xx_cfg80211_change_sta(struct wiphy *wiphy,
				      struct net_device *dev,
				      const u8 *mac,
				      struct station_parameters *params)
{
...
int CFG80211_OpsSuspend(struct wiphy *wiphy, struct cfg80211_wowlan *wow)
{
	...
	if(wow)
	{
		CFG80211DBG(RT_DEBUG_OFF, ("Trigger\n\r\tany: %d\n\r\tdisconnect: %d\n\r\tmagic_pkt: %d\n\r\tpattern_num: %d\n\r\tpatterns: %p\n",
			wow->any,
			wow->disconnect,
			wow->magic_pkt,
			wow->n_patterns,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,1,0))
		CFG80211DBG(RT_DEBUG_OFF, ("\r\teap_identity_req: %d\n\r\tfour_way_handshake: %d\n\r\tgtk_rekey_failure: %d\n\r\trfkill_release: %d\n",
