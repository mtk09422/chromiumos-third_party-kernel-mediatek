	...
	else if (CFG_P2PCLI_ON(pAd))
	{
		...
		if (pEntry && pEntry->CFGP2pInfo.NoADesc[0].bValid == TRUE)
		{
...
#if 0 // yiwei useless
				pAd->cfg80211_ctrl.GONoASchedule.NextTimePointForWMMPSCounting = pEntry->CFGP2pInfo.NoADesc[0].NextTargetTimePoint;
				pEntry->CFGP2pInfo.NoADesc[0].NextTimePointForWMMPSCounting = pEntry->CFGP2pInfo.NoADesc[0].NextTargetTimePoint;
#endif
			...
			else
			{
				CFG80211_P2pStopNoA(pAd, pEntry);
#ifdef RELEASE_EXCLUDE
				DBGPRINT(RT_DEBUG_TRACE,("P2P_PS 4  %s: !!StopGP.  Error Case. shoudn't happen! \n", __FUNCTION__));
#endif /* RELEASE_EXCLUDE */
			...
			else
			{
...
#if 0/*UAPSD_SUPPORT*/
				{
					...
					/* [Testplan 7.1.5] WMM-PS + NoA
					   When Client wake up and receive MoreData before the last sleep,
					   Client must send a QoS Null frame to GO as a re-triggered frame.
                                        */
...
BOOLEAN CFG80211_P2pResetNoATimer(PRTMP_ADAPTER pAd, ULONG DiffTimeInUs)
{
	...
	if (IS_SW_NOA_TIMER(pAd))
	{
		...
		printk("P2P_PS ==========> P2pResetNoATimer, %ld ==> %ld\n", DiffTimeInUs, GPDiff);
		if (GPDiff > 0)
		{
			GPDiff++;
			...
			if (pAd->cfg80211_ctrl.GONoASchedule.bInAwake == FALSE)
			{
				if (GPDiff > 10)
				{
					printk("P2P_PS ==========> P2pPreAbsenTimer, %ld\n", (GPDiff - 10));
					RTMPSetTimer(&pAd->cfg80211_ctrl.P2pPreAbsenTimer, (GPDiff - 10));
				}
			}
		}
	}
#if 0 // yiwei no h/w timmer
	else
	{
...
VOID	CFG80211_P2pStopNoA(PRTMP_ADAPTER pAd, PMAC_TABLE_ENTRY pMacClient)
{
	...
	pAd->cfg80211_ctrl.GONoASchedule.bWMMPSInAbsent = FALSE; /* Set to FALSE if changes state to Awake */
	#endif

	/* If need not resume NoA. Can reset all parameters. */
#if 0 /* yiwei, this is for GO only */
	if (pAd->cfg80211_ctrl.GONoASchedule.bNeedResumeNoA == FALSE)
#endif /* 0 */
	{
...
VOID CFG80211_P2pStopOpPS(PRTMP_ADAPTER pAd)
{
	if (pAd->cfg80211_ctrl.GONoASchedule.bValid == FALSE)
		pAd->cfg80211_ctrl.bKeepSlient = FALSE;
#if 0 //yiwei redundant code!
	if (P2P_TEST_BIT(pAd->cfg80211_ctrl.CTWindows, P2P_OPPS_BIT))
		pAd->cfg80211_ctrl.bFirstTimeCancelOpps = TRUE;
#endif
...
BOOLEAN CFG80211_P2pHandleNoAAttri(PRTMP_ADAPTER pAd, PMAC_TABLE_ENTRY pMacClient, PUCHAR pData)
{
	...
	if (*pData == SUBID_P2P_NOA)
	{
		...
		if (Value < StartTime)
		{
			...
			#if 0 //yiwei useless!
			pAd->cfg80211_ctrl.GONoASchedule.NextTimePointForWMMPSCounting = pMacClient->CFGP2pInfo.NoADesc[0].StartTime;
			#endif
                                 ...
                                 UCHAR wcidindex, UINT32 Sequence)
{
	...
	if (bNoAAttriExist == FALSE)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE,("P2P_PS Debug: %s() %d ===> Get Entry\n", __FUNCTION__, __LINE__));
			...
			if (INFRA_ON(pAd))
			{
				if (!(pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP & WPA_SUPPLICANT_ENABLE_WPS)) //WPS not under goining!
				{
					...
					p2p_wdev = &pApCliEntry->wdev;
    					...
    					DBGPRINT(RT_DEBUG_TRACE, ("CFG Debug: Infra AuthMode =%d, wdev->WepStatus=%d\n",wdev->AuthMode,wdev->WepStatus));
