...
*/
			...
			if (pAd->GONoASchedule.Count > 0)
			{
				...
				NdisCopyMemory(&pCfg80211_ctrl->beacon_tail_buf[bufferoffset+4+bufferoffset2+5] , &pAd->GONoASchedule.Count, 1);
				NdisCopyMemory(&pCfg80211_ctrl->beacon_tail_buf[bufferoffset+4+bufferoffset2+6], &pAd->GONoASchedule.Duration, 4);
				NdisCopyMemory(&pCfg80211_ctrl->beacon_tail_buf[bufferoffset+4+bufferoffset2+10], &pAd->GONoASchedule.Interval, 4);
				NdisCopyMemory(&pCfg80211_ctrl->beacon_tail_buf[bufferoffset+4+bufferoffset2+14], &pAd->GONoASchedule.StartTime, 4);
...
BOOLEAN CFG80211DRV_OpsBeaconAdd(VOID *pAdOrg, VOID *pData)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: SavePyhMode=0x%x, PhyMode=0x%x\n", __FUNCTION__, pAd->CommonCfg.SavedPhyMode, pAd->CommonCfg.PhyMode));
...
#ifdef CONFIG_MULTI_CHANNEL
						DBGPRINT(RT_DEBUG_TRACE, ("%s: InfraCh=%d, pWdev->channel=%d\n", __FUNCTION__, pAd->MlmeAux.InfraChannel, pWdev->channel));
						if (INFRA_ON(pAd) && 
