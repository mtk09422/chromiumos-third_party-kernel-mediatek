...
#ifdef CUSTOMIZED_COUNTRY_REGION_CE_1
VOID CFG80211OS_EnableChanFlagsByBand(
...
{
    INT32 idx = 0;
    struct ieee80211_channel *pChannels = pBand->channels;
    ...
    IN VOID *pChDesc5G)
{
    ...
    if (pChDesc && n_channels > 0)
    {
        ...
        for (ii=0; ii<n_channels; ii++)
        {
            ...
            pTmpCh[ii].center_freq = pCfg80211_CB->Cfg80211_bands[IEEE80211_BAND_2GHZ].channels[ii].center_freq;
        }
#endif /* CUSTOMIZED_COUNTRY_REGION_CE_1 */
        while (pChDesc && pChDesc->FirstChannel)
        {
            ...
            freq_end_mhz = ieee80211_channel_to_frequency(pChDesc->FirstChannel + (pChDesc->NumOfCh-1), 
                                                          IEEE80211_BAND_2GHZ);
#ifdef CUSTOMIZED_COUNTRY_REGION_CE_1
            CFG80211OS_EnableChanFlagsByBand(pTmpCh, n_channels, freq_start_mhz, freq_end_mhz, (UINT32)pChDesc->ChannelProp);
#else /* ! CUSTOMIZED_COUNTRY_REGION_CE_1 */
            flags = 0;
...
#ifdef CUSTOMIZED_COUNTRY_REGION_CE_1
        CFG80211OS_ForceUpdateChanFlagsByBand(&pCfg80211_CB->Cfg80211_bands[IEEE80211_BAND_2GHZ], pTmpCh);
#endif /* CUSTOMIZED_COUNTRY_REGION_CE_1 */
    }
    /* 5GHz rules */
    ...
    if (pChDesc && n_channels > 0)
    {
        ...
        for (ii=0; ii<n_channels; ii++)
        {
            ...
            pTmpCh2[ii].center_freq = pCfg80211_CB->Cfg80211_bands[IEEE80211_BAND_5GHZ].channels[ii].center_freq;
        }
#endif /* CUSTOMIZED_COUNTRY_REGION_CE_1 */
        while (pChDesc && pChDesc->FirstChannel)
        {
            ...
            freq_end_mhz = ieee80211_channel_to_frequency(pChDesc->FirstChannel + ((pChDesc->NumOfCh-1)*4),
                                                           IEEE80211_BAND_5GHZ);
#ifdef CUSTOMIZED_COUNTRY_REGION_CE_1
            CFG80211OS_EnableChanFlagsByBand(pTmpCh2, n_channels, freq_start_mhz, freq_end_mhz, (UINT32)pChDesc->ChannelProp);
#else /* ! CUSTOMIZED_COUNTRY_REGION_CE_1 */
            flags = 0;
            if (pChDesc->ChannelProp & CHANNEL_PASSIVE_SCAN)
                flags |= NL80211_RRF_PASSIVE_SCAN;
...
#ifdef CUSTOMIZED_COUNTRY_REGION_CE_1
            CFG80211OS_ForceUpdateChanFlagsByBand(&pCfg80211_CB->Cfg80211_bands[IEEE80211_BAND_5GHZ], pTmpCh2);
#endif /* CUSTOMIZED_COUNTRY_REGION_CE_1 */
    }
    
#ifndef CUSTOMIZED_COUNTRY_REGION_CE_1
