...
#ifdef CONFIG_OF
extern unsigned long USB_SIF_BASE;
extern unsigned long USB_BASE;
unsigned long USB11_BASE;


/* main clock for usb port0 */
struct clk *usb1_clk_main;
struct clk *univpll1_clk;
static struct regulator *usb1_power;
#define VOL_3300	(3300000)
static const struct of_device_id apusb1_of_ids[] = {
...
void mt65xx_usb11_phy_savecurrent(void)
{
	...
	clk_disable(univpll1_clk);
	clk_unprepare(univpll1_clk);    
}
...
void mt65xx_usb11_phy_recover(void)
{
    ...
    /* enable_pll(UNIVPLL, "USB11"); */
    clk_prepare(univpll1_clk);
	clk_enable(univpll1_clk);
...
void mt65xx_usb11_clock_enable(bool enable)
{
    ...
    if(enable){
        clk_prepare(usb1_clk_main);
        clk_enable(usb1_clk_main);
    }
    else {
        clk_disable(usb1_clk_main);
	    clk_unprepare(usb1_clk_main);
    }
    return;
}
...
int mt_usb11_init(struct musbfsh *musbfsh)
{
    ...
    /* don't need to power on PHY for every resume */
#ifdef CONFIG_OF
    	ret = regulator_set_voltage(usb1_power, VOL_3300, VOL_3300);
    	if (ret) {
    		pr_err("Failed to set regulator, return value:%d!\n", ret);
    		return ret;
    	}
    	ret = regulator_enable(usb1_power);
    	if (ret) {
...
static int mt_usb11_probe(struct platform_device *pdev)
{
	...
	if (!musbfsh) {
        ...
        univpll1_clk = devm_clk_get(&pdev->dev, "univpll1_clk");
        if (IS_ERR(univpll1_clk)) {
        ...
        config->soft_con = true;
        config->dma      = true;
        config->num_eps = 16;
        config->dma_channels = 8;
        pdata->config = config;
#endif
