...
#include <linux/musb/nop-usb-xceiv.h>
...
#ifdef CONFIG_OF
unsigned long USB_SIF_BASE;
unsigned long USB_BASE;

/* main clock for usb port0 */
struct clk *usb_clk_main;
...
static void mt_usb_enable(struct musb *musb)
{
    ...
    if (!mtk_usb_power) {
        ...
        printk("enable UPLL before connect\n");
        mdelay(10);

        usb_phy_recover();
        printk("usb recovery return\n");
...
#ifdef CONFIG_OF
    USB_BASE = (unsigned long) of_iomap(pdev->dev.of_node, 0);
	USB_SIF_BASE = (unsigned long) of_iomap(pdev->dev.of_node, 1);
