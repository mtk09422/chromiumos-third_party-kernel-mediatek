...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
    ...
    #else
		//_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,GPIO_OTG_DRVVBUS_PIN_M_GPIO);
        //_set_gpio_out(GPIO_OTG_DRVVBUS_PIN,GPIO_OUT_ONE);
        #endif
    } else {
...
void musb_id_pin_work(void)
{
	...
	if (1) {
		...
		/* clear session*/
		devctl = musb_readb(mtk_musb->mregs,MUSB_DEVCTL);
        printk("musb_id_pin_work devctl:0x%x\n", devctl);
