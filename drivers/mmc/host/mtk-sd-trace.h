...
TRACE_EVENT(msdc_sdio_irq_enable,
	...
	TP_fast_assign(
		...
		__entry->depth =
		irq_to_desc(gpio_to_irq(host->hw->sdio.irq.pin))->depth;
	),
...
TRACE_EVENT(msdc_data_xfer,
	...
	TP_printk("id=%d: flags=%08X; err=%08X; bytes=%d",
		__entry->id, __entry->flags, __entry->err, __entry->bytes)
);
...
TRACE_EVENT(msdc_sdhc_irq,
	...
	TP_printk("id=%d tag=%08X data=%08X",
		__entry->id, __entry->tag, __entry->data)
);
...
TRACE_EVENT(msdc_command_start,
	...
	TP_printk("id=%d cmd=%d arg=%08X tag=%d",
		__entry->id, __entry->cmd, __entry->arg, __entry->tag)
);
...
TRACE_EVENT(msdc_command_rsp,
	...
	TP_printk("id=%d cmd=%d err=%d rsp=%08X",
		__entry->id, __entry->cmd, __entry->err, __entry->rsp)
);
